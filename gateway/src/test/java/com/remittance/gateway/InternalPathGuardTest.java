...
/**
 * 가드가 실제로 막는가 — 라우트가 있든 없든 (Phase 4).
 *
 * 왜 통합 테스트로는 부족한가
 * `GatewayRoutingTest`에서 `/internal/...`이 404가 나는 것을 확인했지만,
 * 가드를 꺼도 똑같이 404였다. 맞는 라우트가 애초에 없어서다.
 * 즉 그 테스트가 검증한 것은 "지금 안 열려 있다"이지 "막았다"가 아니었다.
 *
 * 가드의 존재 이유는 나중에 누가 catch-all 라우트를 추가했을 때다.
 * 그 상황을 재현하려면 필터를 직접 부르고 체인이 이어지는지를 봐야 한다 —
