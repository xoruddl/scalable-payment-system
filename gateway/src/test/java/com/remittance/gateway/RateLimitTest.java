...
/**
 * 한 사용자가 전체 용량을 다 먹지 못하게 문 앞에서 막는다 (Phase 4).
 *
 * 왜 이 숫자인가
 * 이 시스템의 용량은 100 TPS다(2026-08-29 실측, 종결 p99 4,639ms). 그 위로 받으면
 * 대기행렬이라 지연이 자릿수로 늘고 모든 사용자가 같이 느려진다. 그래서 한 사람이
 * 전체의 10분의 1(초당 10건)까지만 쓰게 한다.
 *
 * Redis를 실제로 띄워서 잰다
 * 인메모리 대체물로 재면 여기서 확인하려는 것이 사라진다 — 카운터가 프로세스 밖에 있어야
 * 게이트웨이를 여러 대 띄워도 "초당 10건"이 지켜진다. 그래서 컨테이너를 띄운다.
 *
 * 왜 {@code @Tag("integration")}인가
 * 컨테이너를 띄우므로 Docker가 필요하다. 다른 서비스의 통합 테스트는
 * {@code AbstractIntegrationTest}를 상속해 태그를 물려받는데, 이 테스트는 상속하지 않아
 * 태그 없이 {@code unitTest}에 섞여 있었다. 그래서 "Docker 불필요"라던 {@code unitTest}가
 * Docker 없는 로컬에서 {@code Could not find a valid Docker environment}로 죽었다
 * (2026-09-05). Testcontainers를 쓰면서 태그가 없던 유일한 테스트였다.
 */
@Tag("integration")
...
class RateLimitTest {
	...
	/**
	 * 이 PR에서 가장 중요한 계약이다. 필터가 조용히 안 걸리면 제한이 없는 것과 같은데,
	 * 그건 설정 한 줄이 틀려도 아무 티가 안 난다는 뜻이라 반드시 눌러봐야 안다.
	 */
	@Test
	...
	/**
	 * 사용자별로 따로 센다. 합쳐 세면 남이 많이 썼다고 내가 막히는데,
	 * 그건 보호가 아니라 새로운 종류의 사고다.
	 */
	@Test
