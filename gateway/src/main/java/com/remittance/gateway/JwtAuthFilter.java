...
/**
 * 누가 보낸 요청인지 게이트웨이에서 한 번만 확인한다 (Phase 4).
 *
 * 여기서 하는 것과 안 하는 것
 * 토큰을 발급하지 않는다. 검증만 한다 — 이 Phase의 주제가 "게이트웨이가 횡단 관심사를
 * 한 곳에서 처리한다"이지 "인증 서비스를 만든다"가 아니기 때문이다. 발급자가 생기면
 * 비대칭 키와 JWKS로 갈아탄다.
 *
 * ★ 들어온 {@code X-User-Id}는 무조건 지운다
 * 토큰에서 꺼낸 값으로 덮어쓰는 게 아니라, 먼저 지우고 나서 넣는다. 공개 경로에서도 지운다.
 * 안 그러면 밖에서 헤더를 그냥 붙여 보내는 것만으로 남이 될 수 있다.
 * 뒤 서비스는 이 헤더를 믿을 수밖에 없으므로, 믿을 수 있게 만드는 것이 여기 책임이다.
 *
 * 그런데 이것도 진짜 경계는 아니다
 * 서비스 포트가 열려 있는 한 게이트웨이를 건너뛰고 헤더를 위조하면 그만이다.
 * `/internal` 문제와 같은 뿌리이고 답도 같다 — Phase 8의 NetworkPolicy로
 * 서비스가 게이트웨이 말고는 아무에게도 대답하지 않게 될 때 완성된다.
 * 여기서 하는 것은 "게이트웨이를 통과한 요청의 신원은 진짜다"까지다.
 */
@Component
public class JwtAuthFilter implements WebFilter, Ordered {
	...
	/**
	 * {@link InternalPathGuard} 다음에 돈다.
	 *
	 * 순서가 뜻을 갖는다 — 내부 경로는 토큰이 유효해도 나가면 안 된다.
