...
/**
 * 무엇을 기준으로 셀 것인가 — 사용자별로 센다 (Phase 4).
 *
 * 왜 사용자인가
 * 로드맵에는 이 항목의 근거가 *"핫 계좌 보호"*로 적혀 있었다. 그런데 그러려면 게이트웨이가
 * 요청 본문을 열어 받는 계좌를 봐야 한다. 그러면 게이트웨이가 도메인을 알게 되고
 * 본문을 버퍼링하느라 논블로킹 이점도 깎인다. 게다가 핫 계좌는 Phase 6에서 잔액 샤딩으로
 * 이미 풀었다(25 → 70 TPS).
 *
 * 그래서 게이트웨이가 이미 아는 것으로만 판단한다 — 토큰에서 꺼내 넣은
 * {@code X-User-Id}다. 본문을 열지 않고, 방금 만든 인증이 여기서 값을 한다.
 *
 * 헤더가 없으면 통과시키지 않는다
 * 이 헤더는 {@link JwtAuthFilter}가 지우고 다시 넣는다. 그러니 라우팅까지 온 요청에
 * 이게 없다는 것은 인증을 지나오지 않았다는 뜻이고, 그건 있을 수 없는 상태다.
 * 그때 키를 비워 통과시키면 제한이 없는 구멍이 되므로, 대신 공용 키 하나로 묶는다 —
