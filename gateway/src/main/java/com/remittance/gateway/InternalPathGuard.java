...
/**
 * `/internal/*`은 게이트웨이를 통해 나가지 않는다 (Phase 4).
 *
 * 왜 라우트를 안 만드는 것으로 충분하지 않나
 * 지금은 `/internal/**`에 맞는 라우트가 없어서 자연히 404다. 그런데 그건
 * "막았다"가 아니라 "아직 안 열었다"이다. 나중에 누가 편의를 위해 catch-all 라우트를
 * 하나 추가하면 그 순간 잔액을 고치는 문이 조용히 열린다.
 *
 * 경계는 없는 것에 기대면 안 되고 있는 것이어야 한다. 그래서 명시적으로 막는다.
 *
 * 이게 진짜 경계는 아니다
 * 서비스 포트(8081~8085)는 여전히 열려 있으므로 게이트웨이를 건너뛰면 그만이다.
 * 진짜 차단은 네트워크에서 해야 한다 — Phase 8에서 NetworkPolicy로 서비스 포트를
 * 클러스터 안으로만 열면 그때 완성된다. 여기서 하는 것은
 * "게이트웨이는 이 문을 열어주지 않는다"는 약속까지다.
 *
 * 404로 답한다. 403은 "거기 뭔가 있다"를 알려주는 셈이라, 없는 것처럼 구는 편이 낫다.
 */
@Component
public class InternalPathGuard implements WebFilter, Ordered {
	...
	/**
	 * 라우팅보다 먼저 돈다. 뒤에 서면 라우트가 먼저 잡아 흘려보낼 수 있다.
	 */
	@Override
	public int getOrder() {
