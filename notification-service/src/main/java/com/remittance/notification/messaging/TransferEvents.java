...
/**
 * 이 서비스가 소비하는 송금 종결 이벤트. Transfer Service가 발행한다.
 *
 * 중간 단계 이벤트(debited/credited/ledger-recorded)는 듣지 않는다. 알림은 끝났을 때
 * 한 번 가는 것이고, 진행 중인 단계마다 알리면 사용자에게는 소음일 뿐이다.
 *
 * 서비스 간 공유 모듈을 두지 않기로 했으므로 계약을 각자 정의한다.
 * 필드 이름이 곧 계약이니 바꿀 때는 발행하는 쪽과 함께 확인해야 한다.
 *
 * {@code @JsonIgnoreProperties(ignoreUnknown = true)}를 붙여 발행하는 쪽이 필드를 추가해도
...
public final class TransferEvents {
	...
	/**
	 * 종결된 송금 한 건. 알림에 필요한 값이 모두 담겨 있어 다른 서비스에 되묻지 않는다 —
