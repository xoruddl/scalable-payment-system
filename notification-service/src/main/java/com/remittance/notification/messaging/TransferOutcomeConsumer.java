...
/**
 * 송금이 끝났다는 소식만 듣고 알림으로 바꾼다.
 *
 * 중간 단계를 듣지 않는 이유는 알림의 성격 때문이다. 사용자에게 필요한 건 결과 하나지,
 * "출금됐습니다 → 입금됐습니다 → 기록됐습니다"가 아니다.
 *
 * 이 서비스는 Saga에 끼어들지 않는다. 아무 이벤트도 발행하지 않으므로, 여기가 통째로
 * 죽어도 송금은 정상적으로 완료된다. 알림이 송금을 막을 수 있다면 그건 잘못 붙인 것이다.
 *
 * 스레드를 파티션 수에 맞춘다 (Phase 6 Step 2)
 * 2026-08-23 측정에서 종결 경로가 전부 lag 0으로 비워진 뒤에도 여기만 밀렸다.
 * 도착이 초당 79건인데 처리가 18건/s라 적체가 20,000건을 넘었고, 그 속도면 비우는 데
 * 20분이 걸린다. "송금은 끝났는데 알림이 한참 뒤에 오는" 상태다.
 *
 * 자원이 모자란 게 아니었다 — 그때 MySQL CPU는 8.7%, 이 서비스는 3.3%였다.
 * 스레드 하나가 메시지당 커밋 네 번(수신자 2명 × 자리잡기 + 보냄표시)을 순차로
 * 기다린 결과다. 커밋 한 번이 약 14ms라 메시지 하나에 55ms를 쓴다.
 *
 * 같은 송금의 소식은 키가 같아 같은 파티션으로 가므로, 스레드를 늘려도
 * 한 송금의 알림 순서는 그대로다. 병렬이 되는 것은 서로 다른 송금끼리다.
 */
@Component
