 ...
 *
 * 이 서비스는 아무것도 발행하지 않는다 — 듣기만 한다. 그런데도 소비하는 토픽을 선언하는
 ...
 *
 * {@code transfer.completed}·{@code transfer.failed}는 Transfer Service도 선언한다.
 * {@link KafkaAdmin}은 이미 있는 토픽을 다시 만들지 않고 파티션이 모자랄 때만 늘리므로 안전하지만,
 * 선언이 어긋나면 큰 쪽이 이긴다 — 파티션 수를 바꿀 때는 양쪽을 함께 확인해야 한다.
 */
@Configuration
