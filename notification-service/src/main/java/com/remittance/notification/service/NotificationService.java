 ...
 *
 * 여기서 지키려는 것 하나 — 같은 소식으로 두 번 알리지 않는다
 * 이벤트는 at-least-once라 같은 소식이 두 번 온다. 그런데 알림은 되돌릴 수 없다.
 * "10만원을 보냈습니다"가 두 번 가면 사용자는 두 번 빠져나간 줄 안다.
 *
 * 순서 — 자리를 먼저 잡고, 보내고, 그다음에 보냈다고 적는다
 *   ① 자리 잡기(PENDING 저장)  ─▶  ② 발송  ─▶  ③ SENT로 표시
 *
 * 한 소식이 만드는 알림들은 ①과 ③을 함께 한다 (Phase 6, 2026-08-26).
 * 완료된 송금은 둘에게 알리는데, 전에는 각각 저장하고 각각 표시해 커밋을 네 번 썼다.
 * 커밋 하나가 평균 47.8ms짜리 공유 관문을 지나므로 그 값이 싸지 않다.
 * 단계를 합치는 게 아니라 같은 단계의 두 건을 묶는 것이라 아래 보장은 그대로다.
 *
 * 이 순서여야 어디서 죽든 안전하다.
 *   - ①과 ② 사이에 죽으면 → 재배달 때 PENDING을 보고 다시 보낸다.
 *   - ②와 ③ 사이에 죽으면 → 재배달 때도 PENDING이라 한 번 더 나간다.
 *       발송과 기록을 원자적으로 묶을 수 없는 이상 남는 창이고, 이쪽을 택한 것이다 —
 *       드물게 두 번 가는 것이 영영 안 가는 것보다 낫다고 봤다.
 *
 * 반대로 ①에서 곧바로 SENT로 적으면 ①~② 사이에 죽었을 때 재배달이 "이미 보냈다"로 읽고
 * 건너뛴다. 알림이 조용히 사라지고 아무도 모른다. 그래서 상태가 두 개 필요하다.
 */
@Service
...
public class NotificationService {
	...
	/** 완료된 송금은 두 사람에게 알린다 — 보낸 사람과 받은 사람은 서로 다른 소식을 받는다. */
	...
	/**
	 * 실패는 보낸 사람에게만 알린다. 받는 쪽은 애초에 오지 않은 돈이라 알릴 일이 없다 —
	...
	private void deliver(List<NotificationDraft> drafts) {
		...
		try {
			...
			// finally여야 한다. 둘째 발송이 실패했다고 첫째까지 PENDING으로 남기면,
