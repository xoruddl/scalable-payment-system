 ...
 *
 * ① 멱등성 — 같은 송금 ID로 몇 번을 받아도 돈은 한 번만 들어간다.
 * 보내는 쪽은 우리 DB에 제약을 걸 수 없으므로 이 보장이 곧 계약이다.
 *
 * ② 세 실패의 뜻이 서로 다르다 — 이게 이 서비스를 만드는 이유다.
 * 지금까지 이 시스템의 실패는 한 종류뿐이라
 * 다시 보내도 되는 실패와 보내면 안 되는 실패를 구분하는 연습을 할 자리가 없었다.
 */
@SpringBootTest
