...
/**
 * 이 은행의 업무만 한다 — 받고, 판정하고, 기록한다.
 *
 * 결함 주입이 여기 없는 이유 ★
 * 처음에는 "응답을 삼킨다"를 이 안에서 예외로 던졌다. 그랬더니
 * 트랜잭션이 함께 롤백되어 입금이 남지 않았다. 테스트가 잡아냈다.
 *
 * 그러면 이 서비스의 존재 이유가 통째로 사라진다. 타임아웃이 "아무 일도 안 일어남"이면
 * 보내는 쪽은 그냥 다시 보내면 되고, "들어갔나 안 들어갔나"가 생기지 않는다.
 *
 * 그래서 응답을 삼키는 것은 트랜잭션 밖에서 한다({@code CreditController}).
 * 층을 나눠 놓고 보면 당연하다 — 삼키는 것은 전송의 문제이지 업무의 문제가 아니다.
...
class CreditService(
	 ...
	 *
	 * 보내는 쪽은 우리 DB에 제약을 걸 수 없다. **이 보장이 곧 계약이다.**
	 */
	@Transactional
	 ...
	 *
	 * 조회에는 결함을 넣지 않는다. 조회까지 못 믿으면 보내는 쪽에 **아무 수단도 남지 않고**,
