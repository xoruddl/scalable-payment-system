...
- [ ] **리스너 `concurrency`** ← **지금 여기.** 릴레이를 뚫자 `transfer.requested`에
      **lag 13,800**이 쌓였다. account 리스너가 **스레드 1개**로 파티션 3개를 순차 처리 중이다.
      3까지 올릴 수 있고, 그 이상은 파티션을 먼저 늘려야 한다
- [ ] 배치 크기·폴링 간격도 조정 여지가 남아 있다 (지금은 500ms × 100건).
      **컨슈머를 뚫은 뒤 다시 판단한다** — 지금 더 빨리 보내봐야 lag만 커진다
