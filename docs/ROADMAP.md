...
- [ ] **transfer-service 리스너 `concurrency`** ← **지금 여기.**
      account·ledger는 lag 0으로 놀고 있고 **transfer-service 혼자 뒤처리 중인데
      리스너 세 개가 전부 스레드 1개**다. `transfer.ledger-recorded`가 초당 41건이고
      종결이 30건/s — **막힌 구간과 전체 숫자가 처음으로 같은 자릿수**다
