...
- [ ] **입금 리스너가 초당 57건에서 막힌다** ← **지금 여기.** 2026-08-24 60 TPS 실측:
...
- [ ] **입금 한 건 80ms를 줄인다** ← **지금 여기.** 스레드를 2배로 하니 건당 시간이
      ...
      **경합이 MySQL로 옮겨갔다.** 스레드를 더 늘려도 같은 벽에 부딪힌다.
      ...
      의심 순서: **커밋 fsync**(group commit이 되고 있나) → `flush` 2회 →
      Outbox INSERT 2회 → 샤딩하며 늘어난 SELECT 1회
