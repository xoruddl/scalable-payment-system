...
- [x] **잔액 샤딩** ✅ — **핫 계좌 용량 30 → 60 TPS (2배).**
      잔액을 `account_balance_shards`로 옮기고(`366812e`) **락을 조각별로 갈랐다**(`f754b58`).
      락이 계좌 하나면 조각을 나눠도 거기서 다시 줄을 서므로, **쪼개는 것의 절반은 락을 쪼개는 것**이다.
      60 TPS에서 종결 p99 **32.4초 → 3.6초**. 30 TPS에서는 1·2·4·8조각이 전부 같다 —
      **그 부하는 상한 아래라 고칠 게 없다.** 4조각은 60 TPS에서 p99 7.1초로 아직 부족했다
...
- [ ] **접수가 60 req/s에서 막힌다** ← **지금 여기.** 2026-08-24 잔액 샤딩 측정에서
      드러났다 — 8조각 100 TPS를 넣어도 **접수 처리량이 57 req/s**밖에 안 나온다.
      100을 넣으려 해도 받는 쪽이 못 받는다. **핫 계좌를 고치고 나니 병목이 여기로 옮겨왔다.**
      접수 자체의 p99는 368ms로 멀쩡하므로, 막히는 것은 접수 지연이 아니라 **접수 처리량**이다.
      무릎 근처(50~70 TPS)에서 재야 한다 — 400 TPS를 부어놓고 보면 원인이 안 보인다
