...
```
0 → 1 → 2 → 3 → 5(측정) → 6(Saga 용량·정합성) → 4(Gateway) → 6.5(Kotlin) → 7 → 8 → 9 → 10 → 11 → 12 → 13
                 ↑ 앞으로 당김             ↑ 종료 조건 강화                            ↑ 배포는 후순위
```

> **6.5는 나중에 끼운 것**이라 번호가 소수점입니다. 뒤 번호를 밀면
> `phase-N-complete` 태그와 지나간 커밋 메시지의 뜻이 달라집니다.

두 번 움직였고, Phase 6의 종료 조건도 한 번 바꿨습니다.
...
## Phase 6.5. 한도(limit) 서비스 — **Kotlin으로 만든다**

> **왜 여기인가** — Phase 4 다음, Phase 7 전입니다. Gateway를 만들 때 **라우팅 대상이 확정돼
> 있는 편**이 낫고, 컨테이너화 전에 **서비스 구성을 확정**해두어야 Dockerfile을 두 번 안 씁니다.
> (Phase 4가 뒤로 간 이유와 같은 논리입니다.)
>
> **번호를 6.5로 둔 이유**: 뒤 번호를 밀면 `phase-N-complete` 태그와 지나간 커밋 메시지의
> 뜻이 달라집니다. Phase 4를 뒤로 옮길 때와 같은 판단입니다.

### 왜 새 서비스이고, 왜 Kotlin인가

**기존 서비스를 Kotlin으로 다시 쓰지 않습니다.** 배우는 것이 없고, 오늘까지 쌓은 측정
기준선(용량 40 TPS, 락 보유 21ms 등)이 통째로 흔들립니다. **새 바운디드 컨텍스트를 엽니다.**

**언어를 섞는 데 드는 비용이 거의 없습니다.** 서비스끼리 Kafka 이벤트(JSON)와 HTTP로만
이야기하고 각자 별도 프로세스·별도 DB입니다 — **Java 객체를 주고받는 곳이 한 군데도 없습니다.**
그래서 이건 "Kotlin을 끼워 넣는" 작업이 아니라 **MSA가 원래 그렇다는 것을 확인하는** 작업입니다.

한도 계산은 **일·월 누적, 계좌 등급별 규칙, 예외 한도**처럼 분기가 많습니다.
`data class`와 `when`, 그리고 널 안전성이 실제로 값을 하는 종류의 코드입니다.
...
- [ ] `limit-service` 모듈 추가 (포트 8086, `limit_db`)
- [ ] 일·월 한도 검사 — 송금 접수 경로에 참여시킨다
- [ ] **이벤트 계약은 기존과 똑같이 JSON.** 언어가 달라도 계약만 맞으면 된다는 것을 확인
- [ ] **용량을 다시 잰다** — 서비스가 하나 늘면 **40 TPS가 얼마가 되는지**.
      홉이 하나 늘어나므로 종결 지연도 그만큼 늘어난다. **그 대가를 숫자로 남긴다**
...
- [ ] 함께 손봐야 하는 곳: `scripts/homelab-services.sh`의 `SERVICES`,
      ...
      그리고 **"다섯 서비스"가 박혀 있는 문서·주석 10곳** — 여섯이 되는 순간 전부 틀린 말이 된다
