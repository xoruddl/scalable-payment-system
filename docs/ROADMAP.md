...
- [x] **접수를 한 트랜잭션으로 합쳤다** ✅ — `transfer_db` **6.05 → 5.06회** (`7db3622`).
      ...
      **그런데 용량은 그대로 60 TPS다** — 11회 중 1회는 9%라 재현 편차(3,130~4,376ms)에 묻힌다.
...
- [ ] **알림의 커밋 3.93회를 줄인다** ← **지금 여기.** 커밋 전체의 **39%**를 알림이 쓴다
      (알림 2건 × 선점+발송). 그런데 **알림은 종결 경로가 아니다** —
      잎사귀가 같은 MySQL 커밋 관문을 39% 쓰고 있다. 지금 가장 큰 덩어리다
- [ ] ~~송금 1건당 커밋 10.5회를 줄인다~~ — 실제로는 **11.1회**였다(알림 lag 때문에 덜 세어졌다).
      지금은 **10.1회**.  아래는 원래 적어둔 배경이다. 커밋 하나가 47.8ms짜리 관문을
