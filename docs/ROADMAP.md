...
- [ ] **⚠️ 먼저: `LockAcquisitionException`이 DLT로 가지 않게 한다** ← **지금 여기.**
      **지금 운영 설정(DISTRIBUTED)에서도 송금 9건이 `DEBIT_COMPLETED`로 갇힌다.**
      락을 못 잡은 건 "지금 붐빈다"는 뜻이지 처리 못 할 메시지가 아닌데,
      재시도 3회를 넘기면 죽여버린다. **`spread`에서는 안 보이고 핫 계좌에서만 드러난다**
- [ ] **계좌별 전략 분리** — 위 실험이 근거다. 일반 계좌는 낙관적 락, 핫 계좌는 분산 락
