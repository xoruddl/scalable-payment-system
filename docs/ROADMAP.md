...
- [ ] **notification이 혼자 밀린다** ← **지금 여기.** 종결 경로 밖이지만 처리 **8.7건/s**에
      도착 **79건/s**라 lag 24,612 — **비우는 데 50분**이다. 메시지 하나가 **커밋 4번**이고
      (수신자 2명 × `claim` + `markSent`) p95가 247ms다.
      스레드만 늘리면 26건/s라 모자라고 **커밋 수를 줄여야 한다**
