...
- [ ] **Kafka 파티션 3 → N** — 스레드는 파티션 수를 넘을 수 없다.
      **위 상한의 직접 원인**이고, `concurrency`를 올리려면 여기부터다.
      ⚠️ 파티션을 늘리면 키 → 파티션 배정이 바뀐다. **토픽이 빈 상태에서** 해야 한다
- [ ] **입금 한 건 52.5ms를 줄인다** — 파티션을 늘려도 이 값이 상한을 정한다.
      의심할 곳: 커밋 fsync, `processed_events` INSERT + flush, Outbox INSERT 2회
