...
- [ ] **접수가 약 50 req/s에서 포화한다** ← **지금 여기.** 2026-08-24 잔액 샤딩 측정에서
      드러났다 — 8조각 기준 50 TPS를 넣으면 접수가 **48.7 req/s**로 따라오는데,
      60 TPS를 넣으면 **50.2 req/s**밖에 안 나온다. **핫 계좌를 고치니 병목이 여기로 옮겨왔다.**
      접수 자체의 p99는 352ms로 멀쩡하므로 막히는 것은 **지연이 아니라 처리량**이다.
      의심할 곳: 접수 한 건이 트랜잭션 **3개**(키 선점 / 송금+Outbox / 키 결과 기록),
      HikariCP 30개, Tomcat 스레드. 무릎(45~60 TPS)에서 재야 한다
