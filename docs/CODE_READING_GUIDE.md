...
| 행 락 | 분산 락이 먼저 사라진 틈(연장 실패 · 장애 전환 · 폴백) — 락 수명이 트랜잭션과 같아 여기서 막힌다 |
...
| 릴레이 깨우기 | [OutboxRelayLoop](../transfer-service/src/main/java/com/remittance/transfer/outbox/OutboxRelayLoop.java), [OutboxRelayWakeupTest](../transfer-service/src/test/java/com/remittance/transfer/outbox/OutboxRelayWakeupTest.java) | 커밋 전에 깨우면 왜 효과가 사라지는가? 깨우는데도 주기를 남긴 이유는? `@Scheduled` 대신 전용 스레드를 쓴 대가는? |
| 진입점 | [GatewayRoutingTest](../gateway/src/test/java/com/remittance/gateway/GatewayRoutingTest.java), [JwtAuthFilterTest](../gateway/src/test/java/com/remittance/gateway/JwtAuthFilterTest.java), [RateLimitWithoutRedisTest](../gateway/src/test/java/com/remittance/gateway/RateLimitWithoutRedisTest.java), [RateLimitRedisDisconnectTest](../gateway/src/test/java/com/remittance/gateway/RateLimitRedisDisconnectTest.java) | 인증과 요청 제한이 장애 시 다르게 동작하는 이유는? fail-open인데도 Redis가 죽으면 요청이 붙들릴 수 있는 이유는? |
