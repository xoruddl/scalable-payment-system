...
새 입금 POST는 외부 전용 Kafka 리스너가 처리하고, 격벽이 전체 동시 호출 수를 제한합니다.
같은 은행이 5번 연속 실패하면 은행별 회로가 30초 동안 열립니다. 이후 한 건만 HALF_OPEN
시험 호출로 보내 복구를 확인합니다. 단, 이미 보낸 돈의 **조회 GET은 회로로 막지 않습니다** —
새 요청을 보호하려다 결과를 모르는 돈의 해소까지 늦추면 안 되기 때문입니다.
