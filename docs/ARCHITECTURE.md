...
**"여러 명이 송금한다"는 앞의 둘이고, 세 번째가 아닙니다.** 계좌가 다르면
**다른 행 · 다른 락 키 · 다른 샤드**라 완전히 병렬입니다. 락이 개입할 일이 없습니다.
...
**같은 시스템인데 4배 차이**입니다. 그래서 상한을 정하는 것도 다릅니다.
...
> **락 방식(낙관적·비관적·Redisson)을 바꾸는 이야기는 아래쪽에만 해당합니다.**
> 위쪽에서는 락을 뭘로 바꾸든 차이가 없습니다.

#### 그럼 이 시스템에서 경합은 어디서 생기나

**"많이 쓴다"가 아니라 "같은 계좌에 몰린다"일 때입니다.**
...
`SettlementAccounts`는 **은행 코드당 정산 계좌를 하나** 둡니다.
...
**한 은행으로 나가는 모든 외부 송금이 계좌 한 개를 거칩니다.** 3,000만 명 규모에서
외부 송금이 20%만 돼도 **하루 600만 건이 그 계좌 하나**에 몰립니다.

> 일반 사용자끼리의 송금은 아무리 늘어도 경합이 안 생깁니다.
> **경합은 "구조적으로 한 곳에 모이는 계좌"에서만** 생기고, 이 시스템에서 그건 정산 계좌입니다.
> 지금은 8조각으로 70 TPS인데, 그때는 **조각을 더 늘릴지 / 배치 집계로 갈지**를 재서 정합니다
