...
Phase 4       🔄 진행 중 — Gateway·Config. **PR을 항목마다 잘게 낸다**(6개 예정)
             ...
             2/6 Config Server 🔄 서버는 떴다(2a). 여섯 서비스가 받아 쓰는 건 2b
             3/6 Gateway 라우팅 · 4/6 인증 필터 ·
...
## Config Server를 살렸다 — 그리고 예상과 반대인 것을 하나 배웠다

**Phase 4 (2/6)** · `feature/phase-4-config-server`

빈 껍데기였던 `config-server` 모듈이 실제로 설정을 냅니다. 저장소는 **이 저장소 자신**이고
설정은 `config/` 아래에 둡니다.

### 왜 별도 저장소로 빼지 않았나

실제 운영은 설정 저장소를 따로 둡니다. 그런데 이 프로젝트는 `build-info`로
**"지금 떠 있는 게 어느 커밋인가"**에 답하도록 만들어뒀습니다. 설정이 다른 저장소에 있으면
**그 답이 반쪽**이 됩니다 — 코드는 알아도 그때 설정이 뭐였는지는 모릅니다.
매니페스트 저장소를 분리할지는 Phase 8의 결정으로 이미 열려 있으니, 옮긴다면 그때 함께입니다.

### ★ `file:` URI는 커밋된 것이 아니라 **작업 트리**를 낸다

설정 파일 주석에 *"커밋하지 않으면 아무것도 안 바뀐다"*고 먼저 써놨는데, **테스트가 그걸
깼습니다.** Git 백엔드는 `file:` URI를 만나면 **clone하지 않고 작업 트리를 그대로 읽습니다.**

```
config/application.yml을 고치고 커밋하지 않음
  → Config Server가 그 변경을 그대로 낸다     ← 예상과 반대
```

그래서 지금 구성에서는 **label(브랜치)이 사실상 뜻이 없습니다.** "그 브랜치에 커밋된 것"을
내려면 `CONFIG_REPO_URI`에 **원격 주소**를 줘야 합니다.

**이 차이를 모르면 두 번 속습니다** — 개발할 때는 *"고쳤는데 왜 반영되지?"*로,
운영에서는 *"커밋 안 했는데 왜 반영됐지?"*로. 테스트로 못 박아뒀습니다.

> 문서에 먼저 쓴 설명이 틀렸다는 걸 **테스트가 알려줬습니다.** 코드를 안 돌려보고 적었으면
> 그대로 남았을 문장입니다.

### 겪은 것

- **Spring Cloud 2025.1.3은 Boot 4.0.8 기준입니다.** 이 저장소는 4.1.0이라 minor가 다릅니다.
  Spring Framework 줄기(7.0.x)가 같아서 붙었고 Boot 아티팩트도 4.1.0으로 해소되는 것을
  의존성 트리에서 확인했지만, **다음 Boot 업그레이드에서 여기가 먼저 깨질 자리**입니다
- **macOS 임시 디렉터리에서 JGit이 거부합니다** — `/var`가 `/private/var`로 가는 심볼릭 링크라
  *"Path component must not be a symbolic link"*가 납니다. `toRealPath()`로 풀었습니다.
  **리눅스 CI에서는 안 나는 문제**라 로컬에서만 red가 됩니다
- Boot 4는 MockMvc 자동 설정이 별도 모듈(`spring-boot-webmvc-test`)이라 `starter-test`만으로는
  안 옵니다. springdoc PR에서 겪은 `TestRestTemplate` 이동과 같은 결의 변화입니다

### 확인

| 무엇 | 결과 |
|---|---|
| Git 백엔드로 뜨는가 | ✅ Spring Cloud 2025.1.3 × Boot 4.1이 실제로 붙는다 |
| 설정을 내는가 | `/application/default`가 임시 저장소에 커밋한 값을 돌려준다 |
| `file:` URI의 실제 동작 | **작업 트리를 낸다** (커밋 안 한 변경도 나간다) |

**테스트가 자기 git 저장소를 만들어 씁니다.** 이 저장소를 읽게 두면 *"지금 어느 브랜치에
무엇이 커밋돼 있는지"*에 테스트가 의존하고, 설정 파일을 추가하는 PR에서 red가 됩니다 —
코드가 틀려서가 아니라요.

### 아직 아무도 안 쓴다

여섯 서비스가 이걸 받아 쓰는 것과 홈서버 배선은 **다음 PR(2b)**입니다.
그래서 지금은 **뜨고 응답하지만 소비자가 없습니다** — 쪼갠 대가이고, 다음 PR에서 붙습니다.
