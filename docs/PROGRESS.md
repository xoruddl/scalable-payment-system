...
Phase 6  🔄  고동시성 대응   ← 지금 여기
             ...
             잔액 샤딩     ✅ 핫 계좌 용량 25 → 50 TPS (2배)
                              → 다음: 접수가 약 50 req/s에서 포화 (병목이 계좌 밖으로)
Phase 7~13   미착수
```
...
| **용량 (핫 계좌)** | **50 TPS** (8조각) — 쪼개기 전 25 TPS에서 **2배** |
| 종결 처리량 | **80건/s** (baseline 19에서 4.2배) — 밀린 큐를 비우는 속도 |
| 다음 병목 | **접수가 약 50 req/s에서 포화** — 핫 계좌를 고치니 여기로 옮겨왔다 |
...
그 실행의 **접수 처리량이 50.19 req/s**입니다. 60을 넣으려 해도 받는 쪽이 못 받습니다.
50 TPS에서는 48.74로 거의 따라가는데 60에서 못 따라가므로, **접수가 약 50 req/s에서
포화**합니다. 쪼갠 목적은 달성됐고 **병목이 계좌 밖으로 옮겨갔습니다.**
