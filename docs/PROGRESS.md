...
Phase 6  🔄  고동시성 대응   ← 지금 여기
             ...
             잔액 샤딩     ✅ 핫 계좌 용량 30 → 60 TPS (2배)
                              → 다음: 접수가 60 req/s에서 막힌다 (병목이 계좌 밖으로 옮겨감)
Phase 7~13   미착수
```
...
| **용량 (핫 계좌)** | **60 TPS** (8조각) — 쪼개기 전 30 TPS에서 **2배** |
| 종결 처리량 | **80건/s** (baseline 19에서 4.2배) — 밀린 큐를 비우는 속도 |
| 다음 병목 | **접수 처리량 57 req/s** — 핫 계좌를 고치니 여기로 옮겨왔다 |
...
## Phase 6 Step 1 — 잔액 샤딩: 핫 계좌 용량 30 → 60 TPS ✅ ★★
...
**핫 계좌 용량 30 → 60 TPS (2배).** 60 TPS에서 1조각 대비 **p99 32.4초 → 3.6초, 8.9배.**

30 TPS에서는 조각을 아무리 나눠도 똑같습니다 — **그 부하는 상한 아래라 고칠 게 없습니다.**
샤딩은 상한에 부딪힌 뒤에만 의미가 있고, 이 표가 그걸 그대로 보여줍니다.

### 4조각으로는 부족했습니다

60 TPS에서 4조각은 p99 7.1초로 **여전히 SLO를 못 지킵니다.** 조각 수는 "나누면 나눈 만큼"이
아니라 **부하가 상한 아래로 내려갈 때까지** 올려야 하는 값입니다. 4조각은 상한을 약 4배로
올렸지만 60 TPS는 아직 그 위였습니다.

### 100 TPS에서는 병목이 옮겨갔습니다

8조각 100 TPS에서 **접수 처리량이 57 req/s밖에 안 나옵니다.** 100을 넣으려 해도 받는
쪽이 못 받습니다. 이제 병목은 계좌가 아니라 **파이프라인**입니다 —
쪼갠 목적은 달성됐고, 다음 표적이 바뀌었다는 뜻입니다.

### ⚠️ 첫 측정을 하마터면 그대로 쓸 뻔했습니다 ★
