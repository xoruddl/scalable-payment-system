...
> **2026-09-15 커밋이 Outbox 릴레이를 깨운다**: 릴레이가 500ms 폴링만 하던 것을, Outbox 행이 커밋되면 곧바로
...
> 1.1초에 DOWN, 게이트웨이 요청은 1.1초에 지나갑니다 — **대신 Redis가 죽어 있는 동안 게이트웨이 요청마다 1초가
> 붙습니다**(그 구간 동안 접수 SLO 500ms를 깹니다). account-service **270건** · gateway **20건** 통과.
> 자세한 것은 아래 "Redis 명령 타임아웃 — 붙들린 것은 health만이 아니었다" 참고.
>
> **2026-09-14 Redis 장애 시험 1**: 홈서버에서 부하(핫 계좌 8조각 40 TPS) 중에 Redis를 약 62초 껐습니다.
> **송금은 멈추지 않았고 종결 p99도 기준선과 같았습니다**(2,047 vs 2,048ms) — 폴백 5,397건 · 대사 0 · 풀 pending 0.
> D-006을 뒤집는 조건은 오지 않았습니다(40 TPS에서). 대신 **`/actuator/health`가 60초 동안 답하지 않는 것**을
> 찾았습니다 — health용 Lettuce에 명령 타임아웃이 없었습니다. 고치는 것은 따로 냅니다.
> 자세한 것은 아래 "Redis 장애 시험 1" 참고. **코드 변경은 없습니다.**
>
> **2026-09-14 Redisson + 폴백**: 직접 만든 `SET NX PX` 락을 **Redisson**으로 갈아탔고,
> **Redis가 죽어도 송금이 멈추지 않게** 했습니다 — 락을 잡는 단계에서 Redis에 닿지 못하면 행 락만으로
> 진행하고(폴백), 연달아 실패하면 회로를 열어 부르지 않으며, Redis 오류는 DLT로 보내지 않습니다.
> Sentinel(주 1 · 복제 1 · Sentinel 3)도 만들었지만, 폴백 뒤에는 Redis를 쓰는 두 곳이 모두 Redis 없이 맞게
> 돌아 **기본은 단일 Redis로 두고 Sentinel은 실험용 오버레이로 내렸습니다.** 코드는 두 모드를 다 지원합니다.
> **측정이 시킨 교체가 아니고(D-005의 조건은 오지 않음), 장애 전환 시험은 아직입니다** — D-006.
> account-service **264건** · gateway **19건** 통과. 자세한 것은 아래 "락을 Redisson으로 — Redis가 죽어도 멈추지 않게" 참고.
>
> **2026-09-13 Phase 6.7 Step 2·3**: 홈서버에서 DISTRIBUTED ↔ PESSIMISTIC을 비교했습니다
> (핫 계좌 8조각 60·70 TPS). 안전 기준(커넥션 pending · 교착 · 갇힘 · 대사)은 둘 다 통과했고
> **SLO는 판정하지 못했습니다** — 기준선 DISTRIBUTED도 60 TPS에서 SLO를 깼고 드리프트가
> 차이보다 컸습니다. 한때 PESSIMISTIC 단독 + Redis 락 제거까지 구현했다가, 소유자 결정으로
> **`LAYERED`(Redis 락 + 행 락)를 기본값으로** 뒀습니다. **측정이 시킨 선택이 아니고 LAYERED는
> 아직 재지 않았습니다** — ROADMAP에 측정 항목으로 남겼습니다. **핫 계좌 용량 후퇴**(70 TPS가
> 재현되지 않음)는 락과 별개라 따로 쫓습니다. 자세한 것은 아래 "분산 락에 행 락을 더했다" 참고.
>
...
> **500으로 맞추고** 문서·SLO 바닥 계산을 고쳤습니다 — 200에 근거가 없고, 종결 p99에
> 여유가 있어 깎을 이유가 없습니다. 자세한 것은 아래 "폴링 주기를 줄인 적이 없었다" 참고.
> **코드 동작 변경은 없습니다.**
>
> **2026-09-11 문서·주석 정리**: 주석에서 HTML 태그를 걷어내고(211파일), 커밋·PR·주석·클린코드
> 작성 규칙을 `CONTRIBUTING.md`·`CLEAN_CODE.md`에 못 박았습니다. 에이전트 메모리에만 있던
> 규칙 10건도 문서로 옮겼습니다. 문서 문체는 한다체로 통일했습니다(이 일지는 제외).
> 자세한 것은 아래 "문서와 주석을 평문으로 정리했다" 참고. **코드 동작 변경은 없습니다.**
>
> **2026-09-05 문서 보강**: [코드 읽기와 면접 준비 가이드](CODE_READING_GUIDE.md)를 추가했다.
> 구조·기술 결정 문서는 있었지만 코드와 테스트를 따라 읽는 학습 순서는 흩어져 있어,
> 접수 → 잔액 변경 → 정합성 → 종결·보상 → 외부 은행 순서로 연결했다.
> 기존 구조·결정·측정 기록은 각 문서를 참조한다.
...
Phase 6.5 ✅ 상대 은행 (Kotlin, HTTP)  — 완료 (2026-08-28), 태그 `phase-6-5-complete`
...
▶ 다음 작업 (2026-08-30에 정한 순서)
  1. 락 해제 실패 카운터    ✅ 2026-08-30 코드 · **✅ 2026-08-31 측정 완료**.
                              `remittance.lock.release{outcome=released|lost}` + 대시보드 패널.
                              lost = TTL이 작업보다 먼저 끝나 **임계 구역이 겹쳐 돌았다**는 뜻.
                              낙관적 락이 뒤에서 막아주므로 **에러율에도 대사에도 안 나타난다**
                              → **70 TPS·8조각에서 34,200번 놓는 동안 lost 0건.**
                              아래 "락은 안 새고 있었다" 참고
  2. SET NX PX → Redisson    ⏭️ **보류 (2026-08-31, D-005).** 근거였던 watchdog은
                              **막을 문제가 없었다** — 1번이 lost 0건을 냈다. 남은 근거인
                              50ms 폴링은 종결 지연의 지배 요인이 아니다.
                              조건: **lost≠0** 이거나 **대기가 p99의 몫이 될 때**.
                              감시는 이미 붙어 있으므로 조건이 오면 보인다
  3. 2차를 다시 판단         ⏭️ **2번이 서지 않아 질문이 안 바뀌었다.** D-004의 "뒤집는 조건"
                              그대로 둔다
  4. @Scheduled → **ShedLock** + SKIP LOCKED (릴레이는 다르게)   ← **지금 여기**
                              세어보니 10개가 아니라 **7개**였다.
                              **✅ 릴레이 절반 완료** (2026-09-02, `3ebb464`) —
                              account·transfer 두 벌을 `SKIP LOCKED` + READ COMMITTED로.
                              **남은 것: ShedLock 4개** (보관 기간 정리 ×2 ·
                              EOD 대사 · 외부 조회 프로버). `ShardRouter`는 **안 붙인다**
                              + Outbox 인프라 두 벌 공통화 (구조 정리)
                              ⚠️ Phase 8의 전제. 컨테이너화 전에 끝나야 한다.
                              **일정이 조건**이라 미룰 수 없는 유일한 항목이다
  ─────
  별건    AccountService 동시성 방어 축 분리 (225줄·의존성 7) — 2번에 딸려 있었으나
          **측정이 아니라 구조가 근거**라 2번 보류와 무관하게 살아 있다 (ROADMAP)
  그다음  Phase 7 컨테이너화
...
| 종결 지연 **p99 < 5초** | 사용자 체감 | Kafka 3홉 × 릴레이 500ms = **바닥 최대 1.5초**, 3배 여유 |
...
릴레이가 최대 500ms를 기다립니다. **폴링 몫만으로 최대 1.5초**이고, 그 아래로는
구조적으로 못 내려갑니다. smoke에서 10 TPS일 때 종결 p99가 **1,693ms**로 나온 것이
그 바닥과 어긋나지 않는 값이었습니다.

> **2026-09-11에 이 문단의 200ms를 500ms로 고쳤습니다.** 재는 값과 적은 값이 달랐습니다 —
> 아래 "폴링 주기를 줄인 적이 없었다" 참고. 1,693ms를 "정확히 그 바닥값"이라고 썼던 것도
> 함께 고쳤습니다. 바닥은 폴링·Kafka·처리 시간이 섞인 값이라 한 숫자로 딱 떨어지지 않습니다.
...
## 락은 안 새고 있었다 — 34,200번 놓는 동안 lost 0건 ★★

**2026-08-31.** 전날 넣은 카운터(`remittance.lock.release`)를 처음으로 **실제로 켜고 쟀습니다.**
카운터를 넣은 목적이 *"Redisson을 왜 넣는지"*의 근거를 만드는 것이었으므로,
재기 전에는 2번을 시작할 수 없었습니다.

### 측정 조건

| | |
|---|---|
| 어디서 | 홈서버(`home2`). **부하도 서버 안에서** — 집 밖 규칙 (`HOMELAB.md`) |
| 코드 | `d4cf653` (PR #17 머지 직후 `develop`), 7개 서비스 전부 같은 해시 |
| 부하 | `measure-hot-account.sh 70 8` — 핫 계좌 70 TPS · 8조각 |
| 자원 | 서비스 0~9번 코어 · k6 10~11번 코어 |
| 규칙 | **재기동 직후 첫 실행은 버렸습니다.** A/B는 같은 온도에서 잽니다 |

버린 실행의 종결 p99는 6,620ms(SLO 미달), 데워진 뒤 본 실행은 2,714ms였습니다.
**같은 코드·같은 부하에서 2.4배 차이**가 났습니다 — 웜업을 버리는 규칙이 왜 있는지가
이번에도 그대로 나왔습니다.

### 본 측정 결과 — 70 TPS는 여전히 용량이다

```
접수 p95 109.8ms · p99 174.3ms · 미발사 0
종결 p95 2,587ms · p99 2,714ms · 성공률 1.00 · 시간초과 0
드레인   미종결 0 · Outbox 0 · Kafka lag 0
대사     5,646계좌 대조, 어긋남 0건
판정     ✅ 지연·회복·정합성 모두 통과
```
...
```
release{released}   34,200      release{lost}   0      ← 이걸 보려고 만들었다
wait{acquired}      34,200      wait{timeout}   0
평균 대기 10.8ms · 최대 대기 605ms          (LOCK_TTL 3초 · WAIT_TIMEOUT 3초)
```

송금 1건에 락이 **약 2회**(출금·입금) 걸리므로 8,400건 부하에 17,100번이 맞습니다.

### lost 0이 뜻하는 것 — 그리고 뜻하지 않는 것

**뜻하는 것**: 임계 구역이 TTL 3초를 넘긴 적이 **한 번도 없습니다.** 가장 오래 기다린
락이 605ms이니 **여유가 약 5배**입니다. TTL이 먼저 끝나 두 서버가 겹쳐 도는 일은
지금 이 부하에서는 **일어나지 않습니다.**

**뜻하지 않는 것**: *"영원히 안 샌다"*가 아닙니다. GC 정지나 DB 지연이 3초를 먹으면
그때는 샙니다. 다만 **이제는 새면 보입니다** — 카운터가 하는 일이 정확히 그것이고,
그 감시가 붙었다는 것 자체가 이번 작업의 결과입니다.

### ★ 그래서 Redisson의 근거가 바뀌었다

`DECISIONS.md`의 2번은 **watchdog으로 TTL 누수를 없앤다**가 근거였습니다.
**그 누수가 측정에서 0으로 나왔으므로 이 근거는 죽었습니다.** 남은 것은 폴링입니다.

대기 시간 분포(34,200건):

```
     ≤ 1ms   27,077   79.2%   경합 없이 바로 잡았다
   1 ~ 5ms    2,732    8.0%
  5 ~ 50ms       90    0.3%   ← 거의 비어 있다
 50 ~ 100ms   2,774    8.1%
100 ~ 250ms   1,403    4.1%
250 ~ 500ms     122    0.4%
500ms ~ 1s        2
...
**5~50ms가 90건뿐인데 50~100ms에 2,774건이 몰린 것은 우연이 아닙니다.**
`RETRY_INTERVAL`이 50ms라, 한 번에 못 잡으면 **락이 5ms 만에 풀렸어도 50ms를 채우고서야**
들어갑니다. 즉 **한 번에 못 잡은 4,301건(12.6%)이 폴링 간격을 통째로 물었습니다.**
pub/sub이 지울 수 있는 몫이 바로 이것이고, **코드에 적어둔 예상이 데이터에 그대로 찍혔습니다.**

**다만 그 몫의 크기도 정직하게 적습니다.** 종결 p99가 2,714ms인데 폴링이 더하는 것은
12.6%에만 붙는 수십~수백 ms입니다. **락 대기는 지금 종결 지연의 지배 요인이 아닙니다**
(지배하는 것은 여전히 지연 쓰기+commit입니다 — Phase 6의 80ms 구간 측정).

그러면 남는 질문은 하나입니다.

> **아프지 않은 것을 고치는 것인가, 아니면 "손댈 때는 현업 기술로"에 해당하는가?**

이 저장소는 **재고 나서 고친다**는 원칙 때문에 한도 서비스와 Config Server를 잘라냈습니다.
같은 잣대를 들이대면 Redisson은 **지금 근거가 약합니다.** 반대로 `DECISIONS.md`의 방침
*"손댈 때 무엇으로 할지는 현업 기술이 기본"*은 **손대기로 한 뒤에** 적용되는 규칙이라,
손댈 이유 자체를 만들어주지는 않습니다.

**판단은 다음 작업에서 합니다.** 다만 근거가 바뀐 사실은 여기 남깁니다 —
**"watchdog 때문에 넣었다"고는 이제 쓸 수 없습니다.**
...
## 릴레이 2대를 진짜로 띄웠다 — 정합성은 통과, 그런데 종결이 2.4배 느려졌다 ★★★

**2026-09-05.** 바로 위에 적어둔 숙제(*"아직 홈서버에서 2대로 띄워 확인하지 않았습니다"*)를
갚았습니다. **정합성은 완전히 통과했고, 예상하지 않은 곳에서 걸렸습니다.**

### 측정 조건

| | |
|---|---|
| 어디서 | 홈서버(`home2`). **부하도 서버 안에서** — 집 밖 규칙 |
| 코드 | `3139410`, 인스턴스 전부 같은 해시(`status`로 확인) |
| 부하 | `measure-hot-account.sh 70 8` — 핫 계좌 70 TPS · 8조각 |
| 자원 | 서비스 0~9번 코어 · k6 10~11번 코어 |
| 2대 구성 | `REPLICAS="transfer-service=2 account-service=2"` |
| 규칙 | **재기동할 때마다 첫 실행은 버렸습니다** |

### ① 릴레이는 의도대로 동작한다 — 통과

**두 릴레이가 큐를 실제로 나눠 가졌습니다.**

```
account-service   :8081  7,509      transfer-service   :8082  3,468
account-service-2 :8181  7,431      transfer-service-2 :8182  3,972
                  50.2 : 49.8                          46.6 : 53.4
```

**이 숫자 하나가 대안 둘을 배제합니다.** 잠금이 없었다면 양쪽이 같은 행을 집어 합계가
2배가 됐을 것이고, `FOR UPDATE`만 걸었다면 두 번째가 기다려 한쪽으로 쏠렸을 것입니다.
`SKIP LOCKED`라서 **서로 다른 행을 나눠 갖고 동시에** 일합니다.

중복이 정말 없는지는 **증가분**으로 봤습니다(누적값은 과거 실행이 섞여 있어 못 씁니다).

```
Δcompleted      8,521
Δnotifications 17,042  = 8,521 × 2   ← 정확히 두 배. 중복 발행 0
Δledger        17,102  = TRANSFER_DEBIT 8,529 + TRANSFER_CREDIT 8,505 + DEPOSIT 68
                         (DEPOSIT은 k6 셋업의 계좌 개설분)
대사            5,890계좌 대조, 어긋남 0건
드레인          미종결 0 · Outbox 0 · Kafka lag 0
```

> **원장은 중복 탐지기가 못 됩니다.** `transactionId`가 UNIQUE이고 `_id`도 같은 값이라
> 중복이 와도 조용히 걸러집니다. **탐지기 역할을 하는 것은 알림**입니다 — 여기가 정확히
> 2배라는 것이 근거입니다.

### ② 그런데 종결 p99가 2.4배 나빠졌다

| 종결 p99 | **1대** | **2대** |
|---|---|---|
| **concurrency 6** (기본) | **3,631 ms** ✅ · **3,133 ms** ✅ (재측정) | **8,396 ms** ❌ |
| **concurrency 3** | — | **7,613 ms** ❌ |

접수는 멀쩡합니다(p99 176 → 209ms). 성공률 1.00, 시간초과 0, 미발사 0 —
**틀린 건 없고 느려졌습니다.**

### ③ 가설 두 개가 틀렸다

**틀린 가설 ①: "행 락을 Kafka 전송 동안 쥔다."** 바로 위 절에서 제가 남긴 대가입니다.
InnoDB 행 락을 **실행 전후 델타**로 재니 **510건 · 총 9.8초 · 평균 19.2ms · 최대 138ms**.
p99가 4초 이상 늘어난 것을 설명하지 못합니다.

> 처음에는 `SHOW GLOBAL STATUS`의 **누적값**을 보고 "평균 0.75ms니 기각"이라고 적었는데,
> 그건 MySQL 기동 이후 전체라 이번 실행에 귀속시킬 수 없는 값이었습니다. 델타로 다시 쟀습니다.

**틀린 가설 ②: "동시 경쟁자가 늘었다."** `concurrency`가 3인 줄 알고 *"2대 × 3 = 6스레드로
경합이 두 배"*라고 적었는데, **실제 값은 6**입니다(제가 본 `concurrency: 6`은 외부은행 전용
리스너였고 메인은 `remittance.kafka.listener.concurrency`의 기본값 6). 파티션이 6개라
Kafka는 파티션당 컨슈머를 하나만 배정하므로 —

```
1대: 스레드  6, 파티션 6 → 활성 6
2대: 스레드 12, 파티션 6 → 활성 6 + 노는 스레드 6
```

**활성 컨슈머는 양쪽 다 6개로 같았습니다.** 노는 스레드를 없애려고 `2대 × concurrency 3`으로
맞춰 재봤지만 **8,396 → 7,613ms, 9% 개선에 그쳤습니다.**

### ④ 배제한 것들 — 전부 실측

| 의심 | 값 | |
|---|---|---|
| **데이터 볼륨 증가** | 1대를 **나중에** 다시 재니 3,133ms — 오전(3,631ms)보다 **오히려 빠름** | ❌ |
| CPU 경쟁 | 부하 구간 최대 **42%** | ❌ |
| HikariCP | pending **0** (네 인스턴스 전부) | ❌ |
| MySQL 커넥션 | 107 / max 300 | ❌ |
| 메시지당 처리 시간 | 커밋 **33.2ms**(입금)·33.9ms(출금), 나머지 전 구간 2ms 미만 — Phase 6의 47.8ms보다 **빠름** | ❌ |
| 락 보유 | 평균 37.7ms · 최대 280ms — **1대와 같음** | ❌ |
| 락 대기 | 평균 11.6 → 12.7ms, **최대 507 → 2,371ms**, timeout 0 → 3건 | ⚠️ 꼬리만 |

**메시지 한 건을 처리하는 일은 전혀 안 느려졌는데 끝에서 끝까지는 2.4배 느립니다.**
어딘가에 줄이 서고 있는데 **위 지표 어디에도 안 잡힙니다. 원인 미상입니다.**

### ⑤ 또 측정 도구가 틀린 걸 재고 있었다

`cmd_stop`을 replica 목록으로 돌게 짰더니, **`REPLICAS`가 지금 안 켜져 있으면 지난번에 띄운
`-2`가 목록에 안 잡혀 살아남았습니다.** `restart` 뒤 `▶ 7개 전부 UP`을 보고 *"1대로 되돌렸다"*고
믿었는데 실제로는 9개가 떠 있었고, **`-2`는 이전 `SERVICE_ENV`의 `concurrency=3`을 그대로
들고 있어 인스턴스마다 설정이 달랐습니다.** 그 위에서 잰 값은 버렸습니다.

**낡은 jar로 재고 전부 버렸던 2026-08-22와 같은 종류입니다** — 커밋 해시는 맞는데 구성이
다른 것을 재고 있었습니다. 이름으로 훑어 전부 내리도록 고쳤고, 확인 방법(발행량이 갈리는지)을
`HOMELAB.md`에 적었습니다.

### ⑥ Phase 8에 남기는 것

Phase 8 체크리스트에 **HPA로 확장성 검증**이 있습니다. **지금 그대로 켜면 핫 계좌 워크로드는
더 느려집니다.** 이건 *"서버를 늘려도 안 풀리는 병목"*(Phase 6의 주제)이 **replica 수준에서
재현된 것**이고, 원인을 모른 채 스케일 아웃하면 **숫자가 나빠지는 것을 보고도 왜인지 말할 수
없습니다.**

**다음 한 걸음**은 변수를 반으로 줄이는 것입니다 — `account만 2대` vs `transfer만 2대`.
그다음이 릴레이 격리(2번 인스턴스만 `outbox.relay.enabled=false`)인데, 그건 `SERVICE_ENV`를
인스턴스별로 주는 확장이 필요합니다.

---

## 느린 게 아니라 무너진다 — 같은 구성을 세 번 재고 나서야 보였다 ★★★

**2026-09-05 (이어서).** 위 절의 "2.4배 느리다"는 **한 번씩 재고 내린 결론**이었습니다.
같은 구성을 **연속으로 세 번** 재보니 신호가 완전히 달랐습니다.

### 회차별 종결 p99 (`70 TPS · 8조각`, 재기동 → 워밍업 30 → 70을 연속 3회)

| 구성 | 1차 | 2차 | 3차 | |
|---|---|---|---|---|
| **1대** | 2,135 ms | 3,098 ms | 3,085 ms | ✅ **안정** |
| **account 2대 · 릴레이 2벌** | 6,100 ms | **15,163 ms** | **19,388 ms** | ❌ **발산** |
| **account 2대 · 릴레이 1벌** | 5,499 ms | 2,654 ms | — | 안정 |

**2대는 느린 게 아니라 무너집니다.** 그리고 **두 번째 릴레이를 끄면 안 무너집니다.**

단발 비교로는 *"조금 느리네"*였는데 반복하니 **발산**이었습니다. 이 저장소는 *"재기동 직후
첫 실행은 버린다"*는 규칙을 갖고 있었지만, **"같은 것을 여러 번 재서 추세를 본다"**는
규칙은 없었습니다. 그래서 훨씬 심각한 신호를 하루 종일 놓쳤습니다.

### 쌓이는 곳은 주로 JVM 안이다

3차(19,388ms) 뒤 **데이터는 그대로 두고 재기동만** 하고 다시 쟀습니다.

```
3차           19,388 ms
재기동 후      8,494 ms   ← 대부분 리셋된다
그 사이클 1차   6,100 ms   ← 완전히 돌아오지는 않는다
```

**약 11초가 프로세스와 함께 사라졌습니다.** 쌓이는 것의 대부분은 JVM 안에 있고,
남은 약 2.4초는 데이터 증가 쪽으로 보입니다.

### 어디까지 좁혔나 — 그리고 무엇을 배제했나
...
|---|---|
| 1대 | 2,849 ms |
| **account만 2대** | **6,897 ms** ← 재현 |
| **transfer만 2대** | **2,984 ms** ← 무죄 |

그리고 **릴레이가 기여한다**는 것까지 갔습니다 — `SERVICE_ENV_2`로 2번 인스턴스의
릴레이만 끄니 6,897 → 5,499ms, **락 보유 시간이 기준선으로 복귀**(39.7 → 36.2ms),
락 대기 timeout 9건 → 1건. 실험이 성립하는지는 2번 인스턴스의
`kafka_producer_record_send_total`이 **0**인 것으로 확인했습니다.

배제한 것은 전부 숫자로 남깁니다.

| 의심 | 반증 |
|---|---|
| 행 락을 Kafka 전송 동안 쥔다 | 실행 전후 델타 510건 · 평균 19.2 ms |
| 동시 경쟁자가 늘었다 | 파티션당 컨슈머는 하나 — **활성 컨슈머는 1대든 2대든 6개** |
| 낙관적 락 충돌 | **0건** (`retried`·`exhausted` 모두) |
| 조각 선택이 인스턴스 로컬 | **이미 무작위** — `ShardRouter`에 그 이유까지 적혀 있다 |
| 데이터 볼륨 | 1대는 더 큰 데이터로도 안정 |
| 워밍업 깊이 | 데워지는 게 아니라 **나빠진다** |
| Redis 포화 | CPU 1.6% · 지연 0~1 ms |
| Outbox 인덱스 | `idx_outbox_unpublished(published_at, id)`로 `rows=1` |
| 버퍼 풀 스래싱 | 적중률 **99.96%** (요청 1.26억 중 디스크 4.9만) |
| GC | 1회 실행 후 **0회** · 힙 141 MB / 1 g |

**원인은 아직 모릅니다.** 여기서 멈춥니다 — 더 파려면 JFR이나 스레드 덤프처럼
다른 도구가 필요하고, 그건 별도 작업입니다.

> **곁가지로 하나**: MySQL 컨테이너가 `mem_limit 1500m`에 **사용 1,436 MiB(98%)**로 붙어
> 있습니다. 버퍼 풀 1,024 MB인데 4개 DB 데이터가 **1,604 MB**입니다. 적중률은 아직
> 99.96%라 당장의 원인은 아니지만, 이 상한들은 **메모리가 15GB이던 시절 값**입니다
> (`HOMELAB.md`의 경고 참고). 31GB 머신에서 다시 잡을 거리입니다.

### ★ 하루에 측정 도구 결함을 셋 찾았다

**이게 오늘의 진짜 수확일지도 모릅니다.** 셋 다 *"틀린 값을 맞다고 말하는"* 종류입니다.

| | 결함 | 어떻게 드러났나 |
|---|---|---|
| ① | `cmd_stop`이 `REPLICAS` 없이 불리면 `-2`를 안 내림 | `▶ 7개 전부 UP`을 보고 1대로 믿었는데 9개가 떠 있었고, `-2`는 이전 `SERVICE_ENV`를 들고 있었다 |
| ② | 대사 findings 종류를 안 보고 전부 *"잔액–원장이 어긋났다"* | 내가 죽인 실행의 `STRANDED_IDEMPOTENCY_KEY` 4건을 보고 잔액이 틀린 줄 알았다. 그 4건은 `transfer_id`가 NULL이라 돈이 움직인 적조차 없었다 |
| ③ | SLO는 **p99**인데 threshold는 **p95** | 종결 p99 5,499ms인 실행에 `✅ 이 TPS는 용량이다`가 찍혔다. `capacity.js`·`mixed-bank.js`는 처음부터 p99였고 두 시나리오만 어긋나 있었다 |

②는 `measure-hot-account.sh`가 **주석에는 "docs/SLO.md의 p99"라고 적어두고 실제로는 p95로
판정**하고 있었다는 점에서 특히 나쁩니다 — **코드가 아니라 주석을 믿으면 알 수 없습니다.**

`cmd_stop`·②·③은 고쳤습니다. **네 번째로 고칠 것은 "여러 번 재서 추세를 본다"**입니다.
지금 스크립트는 한 번 재고 판정하므로, 오늘 같은 발산을 구조적으로 놓칩니다.

### Phase 8에 남기는 것

**HPA를 켜기 전에 이걸 풀어야 합니다.** 파드를 늘리면 느려지는 정도가 아니라 **시간이
갈수록 무너지고**, 그건 오토스케일링이 정확히 잘못된 방향으로 반응하게 만듭니다 —
느려지면 파드를 늘리고, 늘리면 더 느려집니다.

---

## 문서와 주석을 평문으로 정리했다 (2026-09-11)

**계기는 질문 하나였습니다** — *"왜 주석에 시맨틱 태그를 쓰는 거야?"*

근거를 찾아보니 **없었습니다.** `AGENTS.md`에도 `docs/`에도 그런 규칙은 없었고,
어느 `build.gradle`에도 **javadoc 태스크가 없어** 태그가 렌더링될 곳 자체가 없었습니다.
렌더링되는 곳은 IDE의 Quick Documentation 팝업뿐인데, 주석은 팝업보다 **소스 그대로**
읽는 시간이 훨씬 깁니다. 적용도 들쭉날쭉해서 **285개 파일 중 56개**만 `<h2>`를 쓰고 있었습니다.

얻는 것은 팝업의 굵은 글씨 하나, 잃는 것은 상시 가독성이었습니다.

### 무엇을 바꿨나

| 대상 | 규모 |
|---|---|
| 주석 HTML 태그 제거 | 211파일 — `<b>` 1146 · `<p>` 492 · `<h2>`·`<h4>` 121 · `<ul>/<li>` 63 · `<pre>` 18 · `<table>` 7 · `<ol>/<li>` 9 · `<br>` 1 |
| 문서 문체 한다체 통일 | 7개 문서 1,051줄 (`AGENTS.md` 98 · `CONTRIBUTING.md` 17 · 나머지 다섯 936) |
| 규칙 신설 | `CONTRIBUTING.md`에 "PR 작성"·"주석 스타일" 절, "커밋 컨벤션"에 길이·서사 제약 |
| 문서 신설 | `CLEAN_CODE.md` — 우아한테크코스 체크리스트 + Spring 예외 3건 |
| 메모리 → 문서 | 규칙 10건을 옮기고 에이전트 메모리를 비웠습니다 |

`{@link}`·`{@code}`는 남겼습니다. HTML 태그가 아니라 IDE에서 동작하는 참조이고,
`{@code /internal/*}`처럼 코드와 산문을 갈라주므로 **가독성을 돕는 쪽**입니다.

### ★ 기계로 밀면 깨지는 자리가 셋 있었습니다

**한국어 어미 변환은 정규식 한 줄로 안 됩니다.** 실제로 쓰인 어미 **434종**을 먼저 뽑아
분류하고 나서야 규칙이 섰습니다.

```
자음 어간   동사냐 형용사냐            잡습니다 → 잡는다    /  있습니다 → 있다
ㅂ니다      받침을 ㄴ으로 바꾸냐 떼냐    합니다 → 한다       /  다릅니다 → 다르다
입니다      앞 글자 받침 유무          지도입니다 → 지도다   /  값입니다 → 값이다
```

이 셋을 안 가른 1차 시도는 `결론짓습니다 → 결론짓다`, `지도입니다 → 지도이다`를 만들었고
**전부 되돌리고 다시 했습니다.**

그리고 **같은 글자가 두 뜻이었습니다.** `겁니다`는 추정(`통과했을 겁니다`)이기도 하고
동사 `걸다`(`락을 겁니다`)이기도 해서, 후자 5곳을 손으로 되돌렸습니다.

**PR #27을 먼저 머지하고 나서 한 번 더 걸렸습니다.** 충돌을 풀며 스크립트를 다시 돌렸더니
`**무죄**입니다 → **무죄**이다`가 나왔습니다 — 받침을 판정할 자리에 한글이 아니라 마크다운
`**`가 있었기 때문입니다. **이미 머지된 문서 5개에 30곳이 같은 상태**였고, 충돌 해소 작업이
없었으면 못 찾았을 것입니다.

### 검증

수동 확인이 아니라 세어서 확인했습니다.

- **코드 줄 변경 0건** — diff의 모든 `+`/`-` 줄에서 주석(`*`, `//`)이 아닌 줄을 걸러 확인
- 남은 HTML 태그 0개, 문서 존댓말(인용문 밖) 0줄, 깨진 문서 참조 0건
- 문체 전환 전후로 **코드펜스·표 파이프·헤딩·링크·줄 수가 전 문서 동일**
- 큰따옴표 안 인용문은 보호했습니다 — 사용자 발언, 면접 답변 예시, 에러 메시지, UI 문구

### 출시와 정리

밀려 있던 PR 넷을 **#25 → #26 → #27 → #28** 순으로 냈습니다.
#27과 #28이 `ROADMAP.md`에서 겹쳐, **재생성이 가능한 #28이 충돌을 흡수**하도록 순서를 잡았습니다
(반대로 하면 24줄 추가가 243줄 재작성을 상대로 손으로 풀어야 합니다).

머지 뒤 브랜치를 **32개 → 3개**로 줄였습니다(`develop` · `main` · `feature/phase-4-config-server`).
지우기 전에 `refs/backup/*` 68개를 남겼습니다. 마지막 하나를 남긴 이유는
**커밋 `1f0c051`이 그 브랜치에만 있고 `ROADMAP.md`가 그 브랜치를 가리키기 때문**입니다.

### 남긴 것

- **`PROGRESS.md`는 존댓말 그대로 둡니다.** 겪은 일을 적는 시간순 일지라 규칙 문서와 성격이 다릅니다
- **주입 필드가 7개인 서비스 셋**(`AccountService`·`TransferSagaService`·`ReconciliationService`)은
  `CLEAN_CODE.md`가 정한 "5개를 넘으면 쪼갤 신호"에 걸립니다. 이번엔 코드를 건드리지 않았고,
  `AccountService` 축 분리는 이미 별건으로 살아 있습니다
- javadoc을 실제로 내기로 하면 **주석 규칙의 전제가 사라집니다.** 그래서 `CONTRIBUTING.md`에
  결론이 아니라 이유를 적어뒀습니다

## 폴링 주기를 줄인 적이 없었다 (2026-09-11) ★★

**계기는 질문 하나였습니다** — *"Outbox 폴링 주기를 줄이면 더 빨라지지 않나?"*

답을 찾다가 문서와 설정이 서로 다른 숫자를 말하고 있는 걸 발견했습니다.

| 자리 | 값 |
|---|---|
| `OutboxRelay`의 `@Scheduled` 기본값 | `${outbox.relay.interval-ms:200}` |
| `application.yml` (account · transfer) | `interval-ms: 500` ← **이쪽이 이깁니다** |
| `ROADMAP.md` | "폴링 간격은 500 → 200ms" |
| `PROGRESS.md` · `SLO.md` | "Kafka 3홉 × 릴레이 200ms = 바닥 1초" |

`f3cc54e`(릴레이 드레인)가 바꾼 것은 **기본값뿐**이었고 커밋 메시지에도 간격 이야기는
없습니다. yml은 Phase 2부터 500이었고 아무도 안 건드렸습니다. Config Server는 아직
Git config repo를 안 붙였고 compose에도 `OUTBOX_RELAY_*`가 없으니, **떠 있던 값은 계속 500ms**였습니다.

### 무엇이 틀렸고 무엇이 안 틀렸나

**틀린 것은 실험 기록입니다.** ROADMAP의 "폴링 간격을 500 → 200으로 줄였는데 종결이
32 → 30건/s로 안 움직였다"는 **줄이지 않은 상태의 관찰**이었습니다.

**안 틀린 것은 결론입니다.** "주기는 처리량에 안 붙는다"는 여전히 맞고, 다만 그 이유가
주기를 줄여봐서가 아니라 **드레인 루프 때문**입니다. 배치가 가득 차면 이어서 비우므로
적체가 있는 동안 릴레이는 주기를 기다리지 않습니다. 그래서 주기를 줄여도 부하 중에는
바뀌는 게 없고, 그 뒤 종결을 30 → 80건/s로 올린 것은 리스너 `concurrency`였습니다.

주기가 정하는 것은 **무부하 지연 바닥** 하나뿐입니다.

### 그래서 500으로 맞췄습니다 — 200으로 내리지 않았습니다

200에 **기록된 근거가 없습니다.** 커밋 메시지에도 문서에도 왜 200인지가 없어,
`AGENTS.md`가 요구하는 *문제 → 대안 → 이유 → 포기한 것*을 채울 수 없습니다.

그리고 **측정이 아프다고 말하지 않습니다.** 종결 p99 목표 5초에 40 TPS 실측 2.6초로
여유가 있습니다. 이 상태에서 숫자만 깎는 것은 D-005에서 Redisson을 보류한 논리를
스스로 어기는 일입니다.

**500을 유지하는 편이 나중 CDC 논거도 선명하게 만듭니다.** ROADMAP의 Debezium 조건은
"폴링이 병목임을 숫자로 보여준 뒤"인데, 미리 깎아두면 그 숫자가 흐려지고 교체 효과도
작게 나옵니다. `DECISIONS.md`가 이미 "간격 조정은 지연과 부하를 맞바꿀 뿐"이라고
기각해 둔 선택지이기도 합니다.
...
| 대상 | 변경 |
|---|---|
| `OutboxRelay` ×2 | 기본값 200 → **500**. yml과 같은 값으로 둡니다 |
| `application.yml` ×2 | 이 값이 **정하는 것(무부하 바닥)과 안 정하는 것(처리량)**을 주석으로 갈랐습니다 |
| `ROADMAP.md` | "500 → 200ms" 삭제, 경위를 인용문으로 남겼습니다 |
| `SLO.md` · `PROGRESS.md` | 바닥 계산을 3홉 × 500ms = **최대 1.5초**로 고쳤습니다 |

코드 동작은 안 바뀝니다 — 기본값은 yml에 가려 쓰이지 않던 값입니다.
**바뀐 것은 코드만 읽은 사람이 갖게 되는 인상**입니다.

### 남긴 것

- **기본값과 yml이 어긋나면 조용합니다.** 기동 로그에도 `/actuator/info`에도 안 나옵니다.
  `AccountLockPolicy`가 전략을 `/actuator/info`에 싣는 이유가 이것인데, 릴레이 주기는
  그 대접을 못 받고 있습니다. 같은 사고가 또 나면 그때 실을 자리입니다
- **바닥을 p99로 말할 때는 평균이 아니라 최악치로 적습니다.** 주기의 평균 대기는 250ms지만
  목표가 p99라 꼬리 쪽을 써야 합니다
- **이 판단이 뒤집히는 조건**: 저부하 종결 p99가 SLO에 근접하거나 바닥 1.5초 자체가
  요구가 될 때. 그때도 답은 200이 아니라 CDC입니다 — 200으로 가면 바닥이 0.6초로
  내려가도 여전히 폴링이고, 한 번 더 같은 고민을 하게 됩니다

## Phase 6.7 Step 1 — 기다리는 자리를 Redis에서 DB로 옮길 준비 (2026-09-11) ★★

**계기는 질문 하나였습니다** — *"송금 도메인이면 정합성을 극한으로 따져야 하니 비관적 락이
낫지 않나? 처리량이 우선은 아닌 것 같다."*

### 먼저 전제를 하나 정리했습니다

**낙관적 락은 정합성을 낮추지 않습니다.** 둘 다 lost update를 막고, 다른 것은 실패 모드입니다
(D-004 ①). 그리고 지금 돈을 지키는 건 Redis 락이 아니라 `@Version` + DB 트랜잭션입니다 —
Redis 락은 정상 경로를 직렬화하는 성능 장치고, 최후 방어선은 낙관적 락입니다.

그래서 *"비관적으로 바꾸면 더 안전해진다"*는 성립하지 않습니다. **바꿀 근거는 따로 있고,
그건 이미 D-004 ⑤에 빚으로 적어둔 것이었습니다.**

> - 장치가 셋이라 이해하기 어렵다. `FOR UPDATE` 하나면 설명이 한 줄이다.
> - Redis가 잔액 경로의 의존성이 된다.
> - ⚠️ 분산 락이 정말 필요한지 자체가 열린 질문이다. 잔액은 `account_db` 한 곳에 있다.

**정합성을 "보장되는가"가 아니라 "증명하기 쉬운가"로 보면 비관적 락이 낫습니다.**
근거를 이렇게 바꿔 적는 게 중요했습니다 — 틀린 근거로 바꾸면 나중에 *"그래서 뭐가
좋아졌지"*에 답을 못 합니다.

### ★ 그리고 앞선 제 설명을 한 군데 고쳤습니다

*"측정상 경합이 낮다(대기 최대 51ms·획득 실패 0건)"*는 `spread` 시나리오 값입니다.
**핫 계좌에서는 경합이 분명히 있습니다** — 대기 p99 95~100ms, 한 계좌 26건/s 상한.

송금 도메인의 경합은 "전체적으로 높다"가 아니라 **한 줌의 계좌에 몰립니다.**
개인 계좌는 한 사람이 같은 순간에 두 건을 보낼 일이 드물고, 정산·가맹점 계좌는 모든 입금이
한 행으로 들어옵니다. 그래서 답이 *"낮다/높다"*가 아니라 **"계좌 성격에 따라 다르다"**입니다.

그 사실이 방향을 오히려 지지합니다 — 경합이 있는 곳에서 **기다리는 전략이 옳습니다.**

### 그런데 지금 기본값이 이미 기다리는 쪽입니다

여기가 이번 판단의 핵심이었습니다.

| | 목표 | DISTRIBUTED | OPTIMISTIC |
|---|---|---|---|
| 종결 p99 (30 TPS) | < 5초 | **2.6초** | 7.0초 ❌ |
| 낙관적 락 충돌 | — | **0건** | 1,123건 |
| 갇힌 송금 (400 TPS) | 0 | 9건 → **0** | **168건** |

`DISTRIBUTED`에서 Redis 락이 하는 일이 **기다리는 것**이라, 충돌이 0건입니다.
그래서 이번 전환의 정확한 의미는 이렇습니다 —

> **안 기다리던 걸 기다리게 하는 것이 아니라, 기다리는 자리를 Redis에서 DB로 옮기는 것.**

바뀌는 건 장치 수(셋 → 하나)와 Redis 의존이지 실패 모드가 아닙니다.

### 무엇을 만들었나

| 자리 | 무엇을 |
|---|---|
| `AccountLockPolicy` | `Strategy.PESSIMISTIC` 추가 · `usesPessimisticLock()` |
| `AccountBalanceShardRepository` | `findForUpdateBy...` 둘 (`@Lock(PESSIMISTIC_WRITE)`) |
| `BalanceShards` | 전략에 따라 잠그는 조회를 고른다. **잠그며 읽는 자리와 보려고 읽는 자리를 갈랐다** |
| `OpeningBalanceExecutor` | 이월도 합을 보고 판단하므로 잠그며 읽는 쪽으로 |
| `KafkaErrorHandlingConfig` | 락 타임아웃(`PessimisticLockingFailureException`)을 **경합으로 분류** |
| `GlobalExceptionHandler` | 같은 상황을 REST에서도 `409 LOCK_TIMEOUT`으로 |
| `application.yml` | `innodb_lock_wait_timeout = 3` |

`AccountService.guarded()`는 거의 안 바뀌었습니다. `PESSIMISTIC`도 분산 락을 안 잡으므로
기존 분기를 그대로 타고, **락이 사라진 게 아니라 트랜잭션 안으로 들어간 것**뿐입니다.

### ★ 안 하면 조용히 터졌을 자리가 셋 있었습니다

**① 읽기 전용 트랜잭션에서 `FOR UPDATE`는 못 돕니다.** `whole()`은 조회 API와 대사도 씁니다.
같은 메서드에 락을 얹었으면 잔액 조회가 통째로 깨졌을 겁니다. 그래서 `wholeForUpdate()`를
따로 두고, **호출부가 고르는 게 아니라 `BalanceShards`가 정하게** 했습니다.

**② InnoDB 기본 락 대기는 50초입니다.** 비관적 락은 기다리는 동안 **커넥션을 쥡니다.**
50초를 그대로 두면 붐비는 계좌 하나가 풀 30을 통째로 말립니다 — 이 전략의 유일한 실패 모드가
정확히 그것이고, 이 저장소가 이미 겪은 병목입니다(pending 193 · 접수 p99 8.9초).
Redis 락의 대기 상한과 같은 **3초**로 맞췄습니다. 두 전략을 같은 잣대로 재려면
기다려주는 시간이 같아야 합니다.

**③ 예외 이름이 바뀌는 것만으로 갇힘이 재발합니다.** `KafkaErrorHandlingConfig.isContention()`이
`LockAcquisitionException`과 `ConcurrentUpdateException`만 경합으로 봅니다. 이게 갇힘 9 → 0을
만든 수정인데, 행 락 타임아웃은 **다른 예외**로 올라옵니다. 그대로 뒀으면 재시도 3번 만에
DLT로 죽어 `DEBIT_COMPLETED`가 다시 쌓였을 겁니다 — **그것도 핫 계좌에서만 드러나는 형태로.**

### 검증

`@SpringBootTest(properties = "account.lock.strategy=PESSIMISTIC")`로 새 시험을 넣었습니다.
기존 `AccountLockStrategyTest`(OPTIMISTIC)보다 **세게 겁니다.**

- 20스레드가 같은 계좌에 입금 → **거절 0건 · 전부 성공 · 합 일치**
  (낙관적 락은 거절이 알려진 대가지만, 비관적 락은 기다리므로 거절이 나오면 결함입니다)
- **낙관적 락 충돌 0건** — 행 락을 잡고 들어갔는데 버전 충돌이 나면 잠그지 않고 잔액을 만진
  경로가 있다는 뜻입니다. `@Version`을 남긴 이유가 이것입니다(방어선 → **탐지기**)
- 입출금 10건씩 뒤섞기 → 교착 없이 끝나고 합 일치. 출금은 전 조각, 입금은 조각 하나를
  잠그므로 순서가 고정돼 있지 않으면 여기서 멈춥니다
- 세션 변수 확인 — `SELECT @@innodb_lock_wait_timeout` = **3**.
  프로퍼티 한 줄이라 오타가 나도 조용히 넘어가므로 값으로 확인합니다

> **테스트 설정이 운영 설정을 가립니다.** `src/test/resources/application.yml`이 main 쪽을
> 통째로 대신하므로, 거기에도 같은 값을 넣기 전까지 락 타임아웃은 **테스트에서 50초**였습니다.
> 이번엔 위 시험이 잡아냈지만, 안 넣었으면 "테스트는 통과하는데 운영만 다른" 상태였습니다.

account-service 전체 **238건 통과 · 실패 0**입니다.

### 아직 안 한 것

**기본값은 그대로 `DISTRIBUTED`입니다.** 스위치만 만들었습니다.
바꿀지는 Step 2의 재측정이 정하고, 판정 순서를 처리량이 아니라 **커넥션 pending → 락
타임아웃·교착 → 갇힘·대사 → SLO → 처리량**으로 두었습니다. 이 전략의 실패 모드가
풀 고갈이기 때문입니다.
...
## Phase 6.7 Step 2·3 — 분산 락에 행 락을 더했다 (2026-09-13) ★★

### Step 2 — 무엇을 어떻게 쟀나

- 같은 jar `b5b343e`(#31 머지 직후 `develop`), 홈서버 **서버 안에서** 부하(서비스 0~9번 코어, k6 10~11번)
- 핫 계좌 **8조각**, 60·70 TPS 고정 도착률
- 순서: DISTRIBUTED(워밍업 60 → 60 → 70) → PESSIMISTIC(같은 순서) → DISTRIBUTED(워밍업 60 → 60 재측정).
  **마지막 재측정은 드리프트를 보려고** 넣었습니다
- 실행 전후로 `INNODB_METRICS`의 락 카운터와 앱 락 지표를 찍고, 실행 구간의 커넥션 pending 최대값을
  Prometheus에서 읽었습니다

| 실행 | 접수 p99 | 종결 p99 | 미발사 | account 풀 pending | active | transfer 풀 pending | 교착 / InnoDB 락 타임아웃 | Redis 락 타임아웃 | 낙관적 충돌 | 대사 |
|---|---|---|---|---|---|---|---|---|---|---|
| D 60 | 1,791ms | 10,493ms | 64 | **0** | 12 | 77 | 0 / 9 | 3 | 0 | 0건 |
| D 70 | 1,456ms | 13,003ms | 56 | **0** | 11 | 62 | 0 / 8 | 0 | 0 | 0건 |
| P 60 | 943ms | 9,374ms | 21 | **0** | 13 | 39 | 0 / 0 | – | **0** | 0건 |
| P 70 | 3,018ms | 9,473ms | 123 | **0** | 13 | 17 | 0 / 1 | – | **0** | 0건 |
| D 60 재 | 595ms | 7,961ms | 7 | **0** | 12 | 6 | 0 / 0 | 2 | 0 | 0건 |

### 판정 — 안전은 통과했고, 속도는 가르지 못했습니다

ROADMAP에 미리 적어둔 순서대로 봤습니다.

1. **account 풀 pending — 둘 다 0.** 이 전략이 무너질 자리로 짚었던 풀 고갈이 나오지 않았습니다
2. **교착 0 / 0.** 락 타임아웃은 둘 다 0이 아닙니다(D는 Redis 3·2건, P는 InnoDB 1건). 전부 경합으로
   분류돼 재시도됐고 갇힘으로 가지 않았습니다. D 쪽 InnoDB 타임아웃 9·8건은 앞쪽 실행에만 있어
   잔액 행 락이 아닌 곳일 수 있습니다 — 원인을 가르지 못했습니다
3. **갇힘 · 대사 불일치 · DLT 0.** PESSIMISTIC의 낙관적 충돌도 0 — 탐지기가 조용합니다
4. **SLO는 판정 불가.** 아래 절
5. 처리량도 판정 불가 — 미발사가 전 실행에서 0이 아니었습니다

### ★ 기준선도 SLO를 깼습니다 — 핫 계좌 용량이 재현되지 않았습니다

08-24에 같은 조건(8조각)으로 60 TPS 3,354ms · 70 TPS 3,094ms로 통과했던 값이 **두 전략 모두에서**
8~13초가 됐습니다. 같이 뜬 것이 셋입니다 — 접수 p99 1~3초, transfer 풀 pending 최대 77, 미발사.
**account 쪽 pending은 계속 0이었으므로 병목이 잔액 락이 아니라 접수 쪽으로 옮겨간 모양입니다.**
인프라를 막 재기동한 뒤라 MySQL이 식은 영향일 수도 있습니다(워밍업 세 번이 전부 22초 안팎).

그리고 **드리프트가 두 전략의 차이보다 컸습니다.** 같은 D 60 TPS가 20분 사이 10.5초 → 8.0초(−24%)로
움직였습니다. P가 조금 빨라 보이는 것은 이 흔들림 안입니다.
`SLO.md`에 경고를 달고 ROADMAP Phase 6.7에 조건부 항목으로 남겼습니다.

### Step 3 — 두 번 결정했습니다

**첫 결정: PESSIMISTIC 단독 + Redis 락 제거.** 1~3순위(안전)는 측정이 통과시켰고 4순위는 가르지
못한 상태에서, 소유자 우선순위(설명 가능한 단순함)로 정했습니다. 코드를 다 바꿔 234건 통과와
되돌림 확인까지 했습니다.

**두 번째 결정: `LAYERED`(Redis 락 + 행 락).** 커밋 전에 *"같이 쓰면 안 되나"*라는 질문이 나왔고,
소유자가 두 락을 함께 쓰기로 정했습니다. 첫 결정의 코드는 버리고, `AGENTS.md` 규칙 5대로
`refs/backup/phase-6-7-step3-remove-redis`(`f768990`)를 먼저 남겼습니다. 문서에는 과정을 그대로 남깁니다.

근거를 정확히 적어둡니다.

- **"정합성이 더 좋아서"가 아닙니다.** 분산 락 + `@Version`도 lost update를 막고, D 실행 다섯 번도
  대사 0 · 갇힘 0이었습니다
- **"경합이 많아서"도 아닙니다.** 경합이 많다는 것은 **낙관적 락을 배제하는** 근거입니다.
  분산 락과 행 락은 둘 다 기다리고, 다른 것은 기다리는 자리뿐입니다
- **두 락은 하는 일이 다릅니다.** Redis 락은 줄을 DB 밖에 세워 대기 중에 커넥션을 쥐지 않게 하고,
  행 락은 락 수명이 트랜잭션과 같아 Redis 락이 TTL로 먼저 풀린 틈을 막습니다. 전에는 그 틈을
  `@Version`이 충돌 → 재시도로 막았습니다 — 막는 자리가 예외 경로에서 정상 경로로 옮겨왔습니다
- **측정이 시킨 선택은 아닙니다.** `LAYERED`가 막으려는 문제(행 락 대기 중 커넥션 점유)는
  PESSIMISTIC 단독 측정에서 나타나지 않았고(pending 0), `LAYERED` 자체는 아직 재지 않았습니다.
  ROADMAP에 측정 항목으로 남겼습니다

### 바꾼 것

| | 무엇 |
|---|---|
| 새로 뒀다 | `AccountLockPolicy.Strategy.LAYERED` — 기본값. `usesDistributedLock()` · `usesPessimisticLock()`이 둘 다 true |
| 바꿨다 | `BalanceGuard` — 락 분기를 `lockAndRun`으로 뺐습니다(들여쓰기 한 단계). Redis 락은 트랜잭션 밖에서 잡으므로 Redis에서 기다리는 동안은 커넥션을 쓰지 않습니다 |
| 새로 뒀다 | `remittance.balance.lock.failure` — 행 락 포기 횟수. `LAYERED`에서 이게 뜨면 Redis 락이 TTL로 풀린 틈이 실제로 있다는 뜻입니다 |
| 새로 뒀다 | `LayeredLockStrategyTest` 3건 — 기본값과 두 락 · 몰려도 전부 성공 · **Redis를 거치지 않은 트랜잭션이 행을 쥔 틈에 들어온 입금이 기다렸다가 새 값을 읽는가** |
| 더했다 | `BalanceShardingTest` 2건 — 행 락도 조각별로 갈리는가(입금은 비켜 가고 출금은 3초 뒤 포기) · `BalanceGuardTest` 1건 · `KafkaErrorHandlingBackOffTest` 1건(행 락 타임아웃도 경합) |
| 더했다 | Grafana "락 획득 실패" 패널에 행 락 포기 쿼리 |

응답 계약 · 설정 · Redis 의존은 그대로입니다.

### 확인

account-service **250건 통과 · 실패 0** (243 + 7).

핵심 시험은 **일부러 망가뜨려 실패하는지** 봤습니다.

| 망가뜨린 것 | 실패한 시험 |
|---|---|
| 기본값을 `DISTRIBUTED`로 (행 락 없음) | `Redis_락이_없는_틈은_행_락이_기다리게_해서_막는다` — 0이어야 할 `@Version` 충돌이 **1** · `기본_전략은_LAYERED이고_두_락을_모두_잡는다` |

첫 실행은 Testcontainers가 Ryuk에 붙지 못해(`Could not connect to Ryuk`) 시험 본문까지 가지
못했습니다. 무효로 보고 다시 돌린 결과입니다. (첫 결정 때 한 되돌림 확인 — 입금이 조각 전부를
잠그게 하면 `입금은_잠긴_조각을_비켜_간다`가 실패 — 은 같은 시험이 `BalanceShardingTest`에 남아 있습니다.)

### 포기한 것

- **장치가 셋입니다** (Redis 락 · 행 락 · `@Version`). Phase 6.7이 원래 줄이려던 것이 이것이었습니다
- **Redis가 잔액 경로에 남습니다.** 요청마다 Redis 왕복(50ms 폴링)도 붙습니다
- **측정 근거 없이 기본값이 됐습니다.** 1조각 핫 계좌를 포함해 PESSIMISTIC ↔ `LAYERED`를 재야 합니다
- **핫 계좌 용량 후퇴의 원인은 아직 모릅니다**

### 작업 방식 — 같은 디렉터리에서 다른 리팩터링이 돌고 있었습니다

측정 도중 원래 작업 디렉터리가 다른 브랜치(`feature/phase-6-7-saga-step-factory`)로 옮겨졌습니다.
그래서 **rsync를 더 하지 않고** 서버에 이미 있던 깨끗한 `b5b343e`로 빌드했고, Step 3 코드는
**별도 워크트리**에서 고쳤습니다. 측정 래퍼는 한 번 틀렸습니다 — `pipefail`에서
`status | grep -q`는 grep이 먼저 끝나 status가 SIGPIPE로 죽고 파이프 전체가 실패로 잡힙니다.
측정이 한 건도 돌기 전이라 오염은 없었습니다.

## 락을 Redisson으로 — Redis가 죽어도 멈추지 않게 (2026-09-14) ★

**계기는 질문이었습니다** — *"Redisson + Redis Sentinel이 적용된 건가?"* 둘 다 아니었습니다.
락은 Phase 2부터 쓴 `SET NX PX` + Lua 자체 구현이었고, Redis는 단일 인스턴스였습니다.
ZooKeeper와 견준 뒤 소유자가 **Redisson + Sentinel**로 정했습니다 (D-006). Sentinel은 폴백을 넣은 뒤
같은 날 **실험용 오버레이로 내렸습니다** — 이 절 끝의 "노드가 너무 많다" 참고.

### 먼저 정한 것 — 이 락은 효율용입니다

LAYERED에서 잔액의 정합성은 행 락이 지킵니다. 그래서 Redis 락은 **새도 되는 효율용 락**이고,
락 자체가 합의로 안전한 ZooKeeper는 과한 보장입니다(Kleppmann, 2016의 efficiency / correctness 구분).
필요한 것은 "절대 안 새는 락"이 아니라 **"Redis가 죽어도 새 주 노드를 따라가는 클라이언트"**였습니다.
Sentinel 장애 전환 중 비동기 복제로 락이 사라지는 순간은 행 락이 막습니다.

### 한 일

| | 무엇 |
|---|---|
| Redisson | 코어 4.7.0만 넣었습니다(스타터 없이, D-002와 같은 방식). `RedissonConfig`가 `spring.data.redis.sentinel.master`가 있으면 Sentinel, 없으면 단일 서버로 붙습니다. Sentinel에서는 읽기도 주 노드에서 합니다(`ReadMode.MASTER`) |
| `DistributedLock` | 내부를 `RLock`으로 바꿨습니다. TTL 인자를 없앴고(watchdog), 대기는 폴링이 아니라 pub/sub입니다. **지표 이름은 그대로**입니다 |
| `lost`의 뜻 | "TTL이 먼저 끝났다" → **"watchdog이 연장하지 못했다"**(Redis 단절 · 장애 전환) |
| watchdog 타임아웃 | **5초**. 기본값 30초면 락을 쥔 인스턴스가 죽는 순간 그 조각이 30초 막힙니다 |
| Sentinel | `docker-compose.sentinel.yml` — 주 6379 · 복제 6380 · Sentinel 26379~26381, 전부 호스트 네트워크, `down-after` 3초, 정족수 2 |
| 서비스 | `REDIS_SENTINEL=1`이면 `homelab-services.sh`가 account와 gateway에 **같은** Sentinel 설정을 넘깁니다. gateway는 코드 변경 없이 설정만 |

### ★ Sentinel과 Docker — 주소가 맞아야 합니다

Sentinel은 클라이언트에게 "지금 주 노드는 여기"라고 주소를 알려줍니다. 브리지 네트워크면 그 주소가
컨테이너 IP라 호스트에서 뜬 서비스가 닿지 못합니다 — Sentinel과 Docker를 함께 쓸 때 가장 흔히 막히는
자리입니다. 전부 호스트 네트워크로 두어 모두가 `127.0.0.1`로 서로를 부르게 했습니다. 리눅스에서만
되는 방식이라 **홈서버에서만 얹을 수 있고**, 노트북과 테스트는 단일 Redis입니다. 코드 경로는 같고, 모드를 고르는
분기만 `RedissonConfigTest`가 고정합니다.

### 의존성

Redisson 코어가 Kryo 5.6.2 · RxJava 3.1.12 · Jackson 2.21.4를 끌고 옵니다. Jackson 2는 Boot의 Jackson 3과
패키지가 달라 함께 있어도 부딪치지 않습니다. **Netty는 Boot BOM이 4.2.15로 고정**하는데 Redisson 4.7.0은
4.2.16 기준입니다 — 테스트는 통과했지만 어긋나 있다는 것을 적어 둡니다.

### 확인

account-service **253건** · gateway **19건** 통과. (250 + `RedissonConfigTest` 3. `DistributedLockTest`는
11건 그대로입니다 — 중복이던 "대기 시간을 넘기면 예외"를 빼고 watchdog 시험을 넣었습니다.)

| 망가뜨린 것 | 실패한 시험 |
|---|---|
| lease를 5초로 고정 (watchdog 끔) | `오래_쥐고_있어도_watchdog이_락을_연장한다` 하나 — 남은 시간 **2,495ms** ≤ 3,000ms |

한 번은 `RedissonConfigTest`가 컴파일되지 않았는데(`toYAML()`의 `IOException`), 그때 보인 결과 파일은
이전 실행의 것이라 버렸습니다.

### Redis가 죽어도 송금이 멈추지 않게 — 폴백 · 회로 · 경합 분류

**계기는 두 질문이었습니다** — *"Redis에 장애가 나면 해결 방법은?"* 그리고 *"송금 시스템에서 송금이
멈추는 건 말이 안 되니 셋 다 하는 게 맞지 않나?"* 저는 ①만 먼저 하고 ②·③은 측정 뒤로 미루자고
했는데, 소유자가 **가용성 요구**로 셋을 함께 넣기로 정했습니다. 근거는 측정이 아니라 도메인입니다.

코드를 보니 구멍이 있었습니다(측정한 것은 아닙니다). Redis 연결 예외는 경합으로 분류되지 않아
1·2·4초 재시도 뒤 **DLT로 갑니다** — Redis가 7초만 멈춰도 송금이 `DEBIT_COMPLETED`에 갇힙니다.
2026-08-23의 갇힘 9건과 같은 모양이고, 분산 락만 쓰던 시절부터 있던 구멍입니다.

| | 무엇 | 막는 것 |
|---|---|---|
| ① 경합 분류 | Redis 오류(`RedisException` · `LockUnavailableException`)를 경합으로 — 횟수 제한 없이 재시도 | 송금이 **갇히는** 것 |
| ② 폴백 | 락을 잡는 단계에서 Redis에 닿지 못하면 **행 락만으로 진행** (LAYERED에서만) | 송금이 **멈추는** 것 |
| ③ 회로 | 연속 3번 닿지 못하면 5초 동안 Redis를 부르지 않는다 | 요청마다 연결 타임아웃을 기다리는 것 |
| 그리고 | Redis 없이도 뜬다(lazy) · 명령 1초 · 재시도 0 · REST는 503 · 폴백 횟수를 센다 | 죽은 채로 재시작하면 못 뜨는 것 · 조용히 폴백하는 것 |

**안전의 근거는 한 줄입니다 — `LockUnavailableException`은 락을 잡는 단계에서만 나옵니다.** 그래서
폴백이 작업을 대신 돌려도 두 번 돌지 않습니다. 작업이 끝난 뒤 놓으러 갔을 때 Redis가 없는 것은 삼키고
셉니다(`release{unreachable}`). 끝난 작업을 실패로 돌려주면 안 되기 때문입니다.

**붐빔은 폴백이 아닙니다.** Redis가 답했는데 남이 쥐고 있었던 것(`LockAcquisitionException`)을 회로에
실패로 세면, 핫 계좌에서 회로가 열려 멀쩡한 Redis를 두고 폴백으로 샙니다. 성공으로 셉니다.

### ★ 시험이 두 번 틀렸고, 두 번 다 시험이 잡았습니다

1. **Redis를 죽인 척이 먹지 않았습니다.** 베이스가 넣는 Redis 주소를 하위 클래스의
   `@DynamicPropertySource`로 덮어쓰려 했는데 적용되지 않아 **진짜 Redis로 돌았습니다.** "폴백이 0이면
   진짜 Redis로 돈 것"이라는 단언을 넣어 둔 덕에 잡혔습니다. 베이스가 건드리지 않는 키 —
   Sentinel 주소 셋을 죽은 포트로 — 로 바꿨고, 그 김에 설정만으로 Sentinel 모드를 고르는지도 확인됩니다
2. **동시 입금 20건 중 19건이 폴백하지 못했습니다.** Redisson은 연결을 늦게 여는 동안 여러 스레드가
   몰리면 연결 실패를 `CompletionException`으로 **감싸서** 던집니다. 맨 바깥만 보고 가르던 코드는 "Redis
   탓인지 모르는 실패"로 보고 그대로 던졌습니다. 원인 사슬을 따라가게 고쳤습니다 — `KafkaErrorHandlingConfig`가
   `ListenerExecutionFailedException`을 다루는 방식과 같습니다. 처음엔 실패 개수만 세서 원인을 몰랐고,
   **실패 원인을 단언 메시지에 싣게** 시험을 고친 뒤에야 보였습니다

### 확인

account-service **264건** · gateway **19건** 통과.

| 망가뜨린 것 | 실패한 시험 |
|---|---|
| 폴백을 끈다 (`LockUnavailableException`을 그대로 던짐) | `RedisDownFallbackTest` 2건 — 동시 20건이 전부 실패 · `LAYERED면_Redis에_못_닿아도_행_락만으로_한_번_진행한다` |

### 아직 안 한 것

- **홈서버 장애 시험 둘.** 부하 중 ① 기본 구성에서 Redis를 죽인다(폴백) ② Sentinel 오버레이에서 주 노드만
  죽인다(전환) — 송금이 계속 종결되는가, 폴백 · `lost` · 행 락 포기가 몇 건인가, 드레인 뒤 대사가 0인가,
  account 풀 pending이 얼마나 오르는가. 절차는 `HOMELAB.md`에 적었습니다. 코드를 서버로 넘기려면 커밋이
  필요해 그 뒤에 합니다. **①이 D-006을 뒤집을지 가르는 시험입니다** — 폴백 구간이 못 버틸 만하면 Sentinel을 기본으로 올립니다
  → **①은 같은 날 했습니다** (아래 "Redis 장애 시험 1"). ②는 아직입니다

### 포기한 것

- **측정 근거 없이 넣었습니다.** D-005가 보류한 이유(`lost` 0, 대기가 p99를 좌우하지 않음)는 그대로입니다
- Redis 클라이언트가 둘입니다 — Redisson(락)과 Lettuce(`/actuator/health`)
- **폴백하는 동안 줄 세우기가 빠집니다.** 요청이 DB에서 기다리므로 커넥션 대기가 늘 수 있습니다 —
  `remittance.balance.lock.fallback`이 0이 아니면 그 상태입니다
- MySQL · Kafka는 여전히 단일 노드입니다 — 폴백이 없애는 것은 **Redis 때문에** 멈추는 경우뿐입니다

### 노드가 너무 많다 — Sentinel을 실험용으로 내렸습니다

**계기는 질문이었습니다** — *"Redis 관련 노드가 너무 많은 거 아닌가?"* 세어 보니 분산 락 하나를 위해
Redis 주 · 복제 · Sentinel 셋, **다섯**이었습니다. Sentinel이 제대로 돌려면 Sentinel 셋과 데이터 노드 둘이
최소라 줄일 수도 없습니다. 그래서 다섯이 무엇을 사 주는지 다시 봤습니다.

| Redis를 쓰는 곳 | Redis가 죽으면 |
|---|---|
| 잔액 락 (account) | 행 락만으로 진행한다 — 폴백. 정합성은 그대로이고 줄 세우기만 빠진다 |
| 요청 제한 (gateway) | 통과시킨다 — fail-open. 원래 그렇게 만들어져 있었다 |

**둘 다 Redis 없이 맞게 돕니다.** 폴백을 넣기 전에는 Sentinel이 "송금이 멈추는 것"을 막는 장치였지만,
폴백을 넣은 뒤에는 "폴백 구간을 몇 초로 줄이는 것"만 남습니다. 게다가 다섯이 한 머신에 있어 머신
장애는 못 막습니다. 소유자가 **기본은 단일 Redis + 폴백, Sentinel은 실험용 오버레이**로 정했습니다.

| | 무엇 |
|---|---|
| 코드 | **그대로입니다.** `RedissonConfig`는 설정에 Sentinel 주소가 있으면 Sentinel, 없으면 단일 서버로 붙습니다 |
| 기본 운영 | 단일 Redis 1대. `docker-compose.sentinel.yml`과 `REDIS_SENTINEL=1`은 **장애 전환을 볼 때만** |
| 바꾼 것 | 문서와 주석만 — Sentinel을 "홈서버 운영 구성"이 아니라 "실험용"으로 적었습니다 |
| 뒤집는 조건 | 기본 구성의 Redis 장애 시험에서 폴백 구간이 못 버틸 만할 때 — 그때는 설정만 바꿔 Sentinel을 기본으로 |

면접에서는 이렇게 말할 수 있습니다: *"Sentinel까지 만들었다가, 폴백을 넣고 나니 Redis가 필수 인프라가 아니게
돼서 실험용으로 내렸다. 운영이라면 관리형 Redis의 다중 AZ로 받을 자리다."*

## Redis 장애 시험 1 — 부하 중에 Redis를 1분 껐다 (2026-09-14) ★

D-006의 전제 — *"Redis가 죽어도 송금은 멈추지 않는다"* — 를 처음으로 부하 아래에서 확인했습니다.
**A안(단일 Redis + 폴백)을 뒤집을지 가르는 시험이었습니다.** 폴백 구간이 SLO를 깨면 Sentinel을 기본으로
올리기로 했었습니다.
...
| | |
|---|---|
| 코드 | `d414ca9` (PR #34 머지) · `LAYERED` · 단일 Redis |
| 부하 | 핫 계좌 8조각 · 40 TPS 고정 2분 (`measure-hot-account.sh 40 8`) — 드레인 · 정식 대사까지 |
| 순서 | 재기동 → 워밍업(버림) → 기준선(Redis 그대로) → 장애(시작 34초에 `docker stop`, 96초에 `docker start` — 약 62초) |
| 관찰 | 장애 실행 동안 5초마다 account 액추에이터(폴백 · 락 · 풀)와 health · readiness |
| 자리 | 집 밖이라 home2로 들어가 **서버 안에서** 부하를 걸었습니다 — 서비스 코어 0~9, k6 코어 10~11 |

40 TPS는 지금 용량(SLO.md)입니다. 용량 안에서 폴백이 버티는지부터 봤습니다.

### 결과 — 멈추지 않았고, 느려지지도 않았습니다

| | 기준선 | 장애 (Redis 약 62초 꺼짐) |
|---|---|---|
| 종결 p99 (p95) | 2,048ms (2,040) | **2,047ms** (2,042) |
| 접수 p99 | 44ms | 45ms |
| 종결 성공률 · 시간초과 · 미발사 | 1.00 · 0 · 0 | **1.00 · 0 · 0** |
| 드레인 · 정식 대사 | 완료 · 8,147계좌 어긋남 0 | **완료 · 8,208계좌 어긋남 0** |
| account 풀 pending 최대 / active 최대 | 0 / 8 | **0** / 13 |
| InnoDB 행 락 대기 (늘어난 수) | +154 | +745 |
| 행 락 포기 · 교착 · 락 타임아웃 | 0 · 0 · 0 | **0 · 0 · 0** |
| 낙관적 락 충돌 · DLT | 0 · 0 | **0 · 0** |
| 폴백 | 0 | 5,397 |

**D-006을 뒤집는 조건은 오지 않았습니다** (40 TPS에서). 폴백의 대가는 DB 쪽 줄로 나타났습니다 — 줄 세우기가
Redis에서 DB로 옮겨 가 행 락 대기가 약 5배, 커넥션 사용이 8개에서 13개로 늘었습니다. 풀 30개에 pending 0이라
여유가 있었습니다.

5초 표본에서 본 것:

- **회로가 요청마다 Redis를 부르지 않게 했습니다.** Redis 미도달 5,397건 중 실제로 불러서 실패한 것은
  **18건**이고, 5,379건은 회로가 열려 부르지 않았습니다. 62초 동안 5초마다 한 번 확인하는 것과 대략 맞습니다
- **해제 실패(`unreachable`) 4건** — Redis를 끄는 순간 쥐고 있던 락입니다. 설계대로 삼키고 셌습니다. `lost`는 0
- **Kafka 발행 속도는 장애 전후로 같았습니다** (초당 150~160건)
- **Redis를 켜고 8초 안에 Redis 락으로 돌아왔습니다.** 다음 표본에서 락 획득이 다시 늘었고 폴백은 멈췄습니다

워밍업(재기동 직후 첫 실행)은 종결 p99 22.4초로 SLO를 깼고, 규칙대로 버렸습니다.

### ★ 발견 — `/actuator/health`가 60초 동안 답하지 않았습니다

장애 동안 `/actuator/health`가 3초 안에 답하지 않았습니다(표본의 `curl -m 3`). readiness는 줄곧 200이었습니다.
부하 없이 다시 재 보니 **account와 gateway 모두 60초 뒤에야** 503 `DOWN`(Redis 항목만 DOWN)을 냈습니다
(60.06초 · 60.04초, 두 번씩 같았습니다). readiness는 1.5ms에 200이었습니다.

원인은 health의 Redis 항목을 보는 Lettuce에 **명령 타임아웃이 없는 것**입니다. `spring.data.redis.timeout`을
주지 않으면 Lettuce 기본값 60초를 기다리고, 끊긴 동안의 명령은 재접속을 기다리며 줄을 섭니다. 락(Redisson)은
1초로 따로 잡아 뒀기 때문에 송금은 영향이 없었습니다. D-006에 *"Redis 클라이언트가 둘이다"*를 대가로 적어
놓고, 둘째 클라이언트의 설정은 보지 않았던 것입니다.

readiness가 Redis를 보지 않게 한 것(D-006 ⑤)은 맞았습니다 — 봤다면 Redis 하나에 account가 트래픽에서
빠졌을 것입니다. gateway는 요청 제한이 같은 클라이언트를 씁니다. 이번 부하는 서비스에 바로 걸어 게이트웨이를
지나지 않았으므로 요청 경로는 보지 못했습니다. 고치는 것과 그 확인은 따로 냅니다.

### 아직 모르는 것

- **한 번씩만 쟀습니다.** 같은 조건을 되풀이하지 않았습니다
- **40 TPS만 쟀습니다.** 기준선도 SLO를 깬 60~70 TPS(Step 2)에서는 DB 쪽 줄이 더 길어질 수 있습니다.
  핫 계좌 용량 후퇴를 찾은 뒤에 봅니다 — 기준선이 SLO를 깨는 곳에서는 판정이 안 됩니다
- **게이트웨이를 지나지 않았습니다.** 요청 제한의 fail-open은 이 시험에 없습니다
- **장애 전환(Sentinel 오버레이)은 아직입니다**
- 서버 안에서 부하를 걸었습니다 — 코어는 나눴지만 부하 생성기와 서비스가 같은 머신입니다

면접에서는 이렇게 말할 수 있습니다: *"부하 중에 Redis를 1분 껐는데 종결 p99가 기준선과 같았다. 회로 덕에
실제 Redis 호출 실패는 18건뿐이었고, 대가는 DB 행 락 대기가 5배 는 것이었다. 그 시험에서 health가 60초
붙들리는 것도 찾았다 — 락 클라이언트만 조였고 health 클라이언트는 기본값이었다."*

## Redis 명령 타임아웃 — 붙들린 것은 health만이 아니었다 (2026-09-14) ★

장애 시험 1에서 찾은 "health 60초"를 고치러 갔다가, **게이트웨이 요청도 같은 이유로 60초 붙들린다**는 것을
알았습니다. D-006은 게이트웨이 요청 제한을 *"Redis가 없으면 통과시킨다(fail-open)"*로 믿고 있었습니다.

### 왜 60초인가

health의 Redis 항목과 게이트웨이 요청 제한은 둘 다 Lettuce로 Redis를 부릅니다. `spring.data.redis.timeout`을
주지 않으면 Lettuce의 명령 타임아웃은 기본값 **60초**입니다. 그리고 연결이 끊긴 동안 들어온 명령은 바로 실패하지
않고 **재접속을 기다리며 줄을 섭니다**(자동 재접속이 켜진 기본 동작). fail-open은 오류가 나야 작동하는데, 그 오류가
60초 뒤에야 납니다.

`RateLimitWithoutRedisTest`는 이것을 잡지 못했습니다 — **처음부터 없는** Redis를 봅니다. 연결을 한 번도 못 연
클라이언트는 바로 실패합니다. 운영에서 Redis가 죽는 것은 **붙어 있다가 끊기는** 쪽입니다.

### 먼저 재현했습니다 — 로컬에서 한 번 헛돌았습니다

| 시도 | 결과 |
|---|---|
| 테스트 전용 Redis에 붙은 뒤 컨테이너를 끈다 | **고치기 전에도 통과** — 재현되지 않았습니다 |
| 홈서버에서 부하 없이 Redis를 끈다 | account · gateway health 모두 **60초** (고치기 전) |
| 테스트에서 컨테이너를 끄지 않고 멈춘다(pause) | **고치기 전 둘 다 실패** (3초 초과) → 고친 뒤 통과 |

로컬(macOS Docker Desktop)에서는 포트 프록시가 재접속을 받자마자 끊어(`Connection reset`) 명령이 바로
실패했습니다. Linux 서버에서는 연결 거부라 명령이 줄을 섭니다. 멈춘 컨테이너는 어디서나 "답이 없는 Redis"가 되고,
두 경우를 끝내는 것은 같은 설정입니다. 첫 결과만 보고 "재현 안 됨"으로 닫았다면 원인을 놓쳤을 것입니다.

### 바꾼 것

| | 무엇 |
|---|---|
| account · gateway `application.yml` | `spring.data.redis.timeout: 1s` — Redisson의 1초(`RedissonConfig.REDIS_TIMEOUT`)와 맞췄습니다. Redisson은 이 키를 읽지 않습니다 |
| `RedisHealthTimeoutTest` (account) | 붙은 뒤 Redis를 멈추면 health가 3초 안에 DOWN. main의 `application.yml`을 직접 읽습니다 — 테스트용 설정 파일이 main 것을 통째로 가립니다 |
| `RateLimitRedisDisconnectTest` (gateway) | 붙은 뒤 Redis를 멈춰도 요청이 3초 안에 지나간다 |

### 확인

account-service **270건** · gateway **20건** 통과. 새 테스트 둘은 타임아웃을 빼면 3초를 넘겨 실패합니다.

홈서버 (부하 없음, Redis를 끄고 잼):

| | 고치기 전 (`d414ca9`) | 고친 뒤 (`876de7f`) |
|---|---|---|
| account `/actuator/health` | 60.06초 → 503 | **1.06초** → 503 DOWN |
| gateway `/actuator/health` | 60.04초 → 503 | **1.08초** → 503 DOWN |
| 게이트웨이를 지나는 요청 | **60.04초** | **1.07~1.09초** (끄고 12초 뒤에도 같음) |
| account readiness | 200 · 2ms | 200 · 2~3ms |

### 포기한 것 — Redis가 죽어 있는 동안 게이트웨이 요청마다 1초

타임아웃은 60초를 1초로 줄였을 뿐, **끊긴 동안의 명령이 줄을 서는 동작은 그대로**입니다. 그래서 Redis가 죽어
있는 동안 게이트웨이를 지나는 요청은 하나하나 1초를 기다린 뒤 fail-open으로 지나갑니다. 그 구간 동안 접수
SLO(p99 500ms)를 깹니다. Redis를 켜고 5초 뒤 표본에서도 1초가 붙었습니다 — 재접속이 될 때까지는 그대로입니다
(몇 초 걸리는지는 재지 않았습니다).

줄이는 방법은 끊긴 동안 명령을 줄 세우지 않고 바로 거절하는 것(Lettuce `DisconnectedBehavior.REJECT_COMMANDS`)입니다.
그러면 1초는 "붙어 있지만 답이 없는" 경우에만 뭅니다. 설정 한 줄이 아니라 클라이언트 옵션 커스터마이저가 필요하고,
게이트웨이를 지나는 부하로 Redis 장애를 재 본 적이 없어 **숫자 없이 넣지 않았습니다** — ROADMAP에 남겼습니다.

잔액 경로(account)는 영향이 없습니다 — 락은 Redisson이 1초로 따로 잡고 있었고, account의 Lettuce는 health만 씁니다.
...
## 커밋이 Outbox 릴레이를 깨운다 — 폴링 대기를 없앴다 (2026-09-15) ★

### 왜

"결제 도메인은 p99 200ms"라는 비교에서 시작했습니다. 그 숫자는 접수(동기) 기준이고, 우리 접수는 08-26 측정에서
60 TPS 이하면 그 안에 들었습니다. 이야기가 "그럼 송금을 동기로 묶어야 하나"로 번졌고 결론은 **묶지 않는다**였습니다 —
서비스마다 DB가 달라 보상은 그대로 필요하고, 가용성이 곱해지고, 핫 계좌의 락 대기가 접수 응답에 들어옵니다.

대신 종결이 느린 이유를 다시 봤습니다. **비동기라서가 아니라 폴링이라서**입니다. 릴레이가 500ms마다 돌아서,
요청이 밀리지 않으면 커밋된 행이 다음 주기까지 기다립니다. 송금 한 건이 Kafka를 세 번 건너므로 최대 1.5초입니다.
40 TPS 종결 p99 2,048ms의 상당 부분이 여기라고 봅니다(나눠 잰 적은 없습니다).

### 무엇을 골랐나

| 후보 | |
|---|---|
| 주기를 줄인다 | 트래픽이 없어도 빈 조회가 늡니다 — SLO.md가 이미 거절한 길입니다 |
| CDC (Debezium) | 폴링 자체가 없어지지만 Kafka Connect를 운영해야 합니다 |
| 커밋 뒤 그 행만 바로 보내기 | 이벤트마다 발행 표시 커밋이 하나씩 늡니다 — 핫 계좌 입금 시간의 63%가 이미 커밋입니다 |
| **커밋 뒤 릴레이를 깨운다** | ← 인프라가 늘지 않고, 보내는 코드가 그대로입니다 |

### 바꾼 것

| | 무엇 |
|---|---|
| `OutboxEvent` (두 서비스) | `@DomainEvents` — `save`되면 신호를 냅니다. 행을 적는 네 곳(transfer 1 · account 3)은 고치지 않았습니다 |
| `OutboxRelayLoop` (새로, 두 서비스) | 전용 스레드입니다. 신호가 오거나 500ms가 지나면 한 바퀴 돕니다. 신호는 **커밋 뒤에만**(`@TransactionalEventListener`) 받고, 자리 하나짜리 큐라 여러 커밋이 한 바퀴로 합쳐집니다. 한 바퀴의 예외는 잡아서 남깁니다. `remittance.outbox.relay.runs{trigger}`로 무엇이 돌렸는지 셉니다 |
| `OutboxRelay` (두 서비스) | `@Scheduled`를 뗐습니다. 한 바퀴 코드는 그대로입니다 |
| `OutboxRelayDrainTest` | 주기를 10분으로 줘 재우던 것을, 루프를 끄는 것으로 바꿨습니다 — 이제 루프는 커밋이 깨우므로 켜 두면 쌓는 동안 먼저 비워 버립니다 |

**왜 `@Scheduled`가 아니라 전용 스레드인가.** 신호를 기다리는 동안 스레드를 쥐어야 합니다. transfer의 스케줄러는
스레드가 하나라, 거기서 기다리면 보관 기간 정리가 영영 못 돕니다. 08-27에 스케줄러 스레드 하나가 붙들려 릴레이가
굶은 적이 있는데, 이번엔 릴레이가 붙드는 쪽이 됩니다.
...
결함을 심은 시험도 그렇게 돌렸습니다.

### 아직 안 한 것 — 홈서버 측정

| 무엇 | 왜 이 조건인가 |
|---|---|
| 40 TPS 종결 p99 전후 | 09-14 기준선이 2,048ms로 안정적이던 조건입니다. 60·70 TPS는 09-13에 재현되지 않아 효과가 흔들림에 묻힙니다 |
| 70 TPS 용량이 줄지 않는가 | 요청이 적당하면 배치가 작아져 트랜잭션이 늡니다. `relay.runs{trigger=wakeup}`과 커밋 수를 함께 봅니다 |

### 포기한 것

- 릴레이 스레드의 생사를 이제 우리가 맡습니다(예외 · 종료)
- 요청이 적당할 때 트랜잭션이 늘 수 있습니다 — 아직 재지 않았습니다
- 줄어드는 것은 무부하 바닥뿐입니다. 용량은 늘지 않습니다

면접에서는 이렇게 말할 수 있습니다: *"결제 p99 200ms와 비교하다가 송금을 동기로 묶어야 하나까지 갔는데, 느린 이유를
따라가 보니 비동기가 아니라 폴링이었다. CDC 대신 커밋 직후 릴레이를 깨우는 것만 넣었다 — Outbox에 먼저 적는 건
그대로라 보장은 안 바뀐다. 주기는 안전망으로 남겼고, 대가로 릴레이가 스케줄러를 떠나 스레드의 생사를 직접 맡게 됐다."*
