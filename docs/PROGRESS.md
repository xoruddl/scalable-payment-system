...
Phase 6  🔄  고동시성 대응   ← 지금 여기
             Step 2 파이프라인 ✅ 7건 (커넥션 풀 · 릴레이 배치 · 리스너 concurrency ×3 ·
                                      원장 인덱스 · 릴레이 드레인)
             Step 1 핫 계좌  🔄 관문 통과 · 락 전략 비교 실험 완료
                              ⚠️ 지금 설정에서도 송금 9건이 갇힌다 (DLT) — 먼저 고칠 것
Phase 7~13   미착수
```
...
| | |
|---|---|
| **용량** | **40 TPS** — SLO를 지키며 견디는 최대 도착률 (`docs/SLO.md`) |
| 종결 처리량 | **80건/s** (baseline 19에서 4.2배) — 밀린 큐를 비우는 속도 |
| 한 계좌 상한 | **약 26건/s** — 락 보유 38ms가 정하는 값. **서버를 늘려도 안 변한다** |

> **둘을 헷갈리면 안 됩니다.** 80건/s는 큐를 비우는 속도이고, 40 TPS는 **약속한 지연 안에
> 서비스할 수 있는 속도**입니다. 앞으로의 개선은 **"용량이 40 → N TPS가 됐다"**로 말합니다.

**측정 환경**: 홈서버(`ssh home1`). **노트북에서 잰 값은 성능 숫자로 쓰지 않습니다** (`HOMELAB.md`).
**빌드 상태**: 🟢 `./gradlew test` 통과 (테스트 460건)
