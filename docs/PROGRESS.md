...
Phase 6  🔄  고동시성 대응   ← 지금 여기
             ...
             80ms 구간 측정 ✅ 지연 쓰기+commit이 평균 49.5ms · p95 114.9ms로 지배
                              → 다음: MySQL group commit·fsync delta 측정
Phase 7~13   미착수
```
...
| **용량 (핫 계좌)** | **60 TPS** (8조각·파티션 6) — 쪼개기 전 25 TPS에서 **2.4배** |
| 종결 처리량 | **80건/s** (baseline 19에서 4.2배) — 밀린 큐를 비우는 속도 |
| 다음 병목 | **지연 쓰기+commit** — 입금 전체 평균 54.6ms 중 49.5ms, p95 130.4ms 중 114.9ms |
...
|---|---:|---:|---:|---:|
| 워밍업 — 버림 | 323ms | 12,729ms | 14,139ms | 0 · 0 |
| **본 측정** | **428ms** | **3,469ms** | **3,889ms** | **0 · 0** |

첫 실행을 버리는 규칙이 다시 값을 했습니다. 같은 jar·같은 부하인데 워밍업 p99는 14.1초,
본 측정은 3.9초입니다. 워밍업을 결과로 썼다면 60 TPS가 다시 실패했다고 잘못 결론 냈습니다.

### 같은 시각의 내부 구간

`transfer.debited`만, Prometheus의 같은 2분 창으로 비교했습니다.

| stage | 평균 | p95 |
|---|---:|---:|
| `deduplication_flush` | 0.89ms | 1.57ms |
| `balance_load` | 1.86ms | 3.89ms |
| `balance_flush` | 0.91ms | 0.97ms |
| `outbox_enqueue` | 1.41ms | 1.78ms |
| **`deferred_writes_and_commit`** | **49.48ms** | **114.92ms** |
| **트랜잭션 전체** | **54.56ms** | **130.35ms** |

평균은 다섯 구간의 합이 전체와 거의 정확히 맞습니다(54.55 ≈ 54.56ms).
**지연 쓰기+commit 하나가 평균의 90.7%**이고, p95에서도 나머지 네 구간은 모두 4ms 아래입니다.
처리 흔적 flush나 샤드 SELECT를 먼저 고치는 것은 틀린 순서였습니다.

중복 이벤트의 `outcome=rolled_back`은 이 측정 창에 없었습니다. 실패한 짧은 트랜잭션이
성공 지연을 낮춰 보이게 만든 결과도 아닙니다. Grafana 신규 패널 두 개도 실제 값을 냈고,
대시보드 쿼리 **25개 전부** 확인을 마쳤습니다.

### 여기서 아직 말할 수 없는 것

MySQL의 `Innodb_log_waits=0`이라 redo log buffer가 차서 기다린 것은 아닙니다. 하지만
`Innodb_os_log_fsyncs` 같은 값은 측정 뒤 누적 스냅샷만 읽었으므로, **group commit이 되고 있는지,
fsync가 49.5ms의 얼마인지는 아직 모릅니다.** 다음 Step은 부하 전후 delta를 함께 수집하고
내구성 설정은 바꾸지 않은 채 group commit·flush 횟수를 먼저 확인하는 단계입니다.
