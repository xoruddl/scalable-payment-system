...
Phase 6  🔄  고동시성 대응   ← 지금 여기
             ...
             redo 100MB→1GB ✅ 용량 60 → 70 TPS (p99 11.8초 → 3.1초)
                              → 다음: 80 TPS가 왜 7.6초인가
             Phase 6 목표   🔄 정합성·내구성을 지키는 Saga 최대 SLO 용량까지 계속
                              전체 대사는 EOD, 운영 이상은 가벼운 상시 감시로 분리 예정
Phase 7~13   미착수 — 컨테이너·K8s는 위 종료 조건을 통과한 뒤
```
...
| | |
|---|---|
| **용량 (흩어진 계좌)** | **40 TPS** — SLO를 지키며 견디는 최대 도착률 (`docs/SLO.md`) |
| **용량 (핫 계좌)** | **70 TPS** (8조각·파티션 6·redo 1GB) — 쪼개기 전 25 TPS에서 **2.8배** |
| 종결 처리량 | **80건/s** (baseline 19에서 4.2배) — 밀린 큐를 비우는 속도 |
| 다음 병목 | **80 TPS 종결 p99 7.6초** — 커밋 수는 이제 제약이 아니다 |

> **둘을 헷갈리면 안 됩니다.** 80건/s는 큐를 비우는 속도이고, 40 TPS는 **약속한 지연 안에
> 서비스할 수 있는 속도**입니다. 앞으로의 개선은 **"용량이 40 → N TPS가 됐다"**로 말합니다.
