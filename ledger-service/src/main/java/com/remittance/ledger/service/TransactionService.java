...
public class TransactionService {
	...
	/**
	 * 잔액 변경 하나를 원장에 남긴다. 같은 변경을 여러 번 받아도 결과가 같다.
	 *
	 * 이벤트는 at-least-once라 같은 변경이 두 번 올 수 있는데, 원장에 같은 줄이 두 번 생기면
	 * 그 즉시 계좌 잔액과 어긋난다. 그래서 문서 ID를 랜덤이 아니라 발행하는 쪽이 고정해 보낸
	 * 분개 항목 ID로 삼는다. 재수신은 같은 _id에 같은 내용을 덮어쓰는 것으로 끝난다 —
	 * 중복 판별 테이블이 필요 없다.
	 *
	 * Step 5a 전에는 "송금 + 계좌 + 방향"이라는 자연키를 썼다. 원장이 송금만 기록하던 시절엔
	 * 그걸로 충분했지만, 이제 송금과 무관한 변경도 들어오고 같은 송금에서 같은 계좌가
	 * 두 번 움직일 수도 있어(출금 → 환불) 자연키가 더는 성립하지 않는다.
	 *
	 * 기록 시각도 발행 시각을 그대로 쓴다. 수신할 때마다 새로 찍으면 재수신 때 값이 달라져
	 * 멱등하지 않다.
	 */
	...
	/**
	 * 이 송금의 원장 기록이 끝났는지. 출금 줄과 입금 줄이 모두 있어야 끝난 것이다.
	 *
	 * 두 줄은 계좌가 달라 서로 다른 파티션으로 오므로 도착 순서가 보장되지 않는다.
	 ...
	 *
	 * 줄이 하나도 없는 계좌도 0으로 반드시 포함한다. 빠뜨리면 대사하는 쪽이
