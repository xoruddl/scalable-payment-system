...
/**
 * Account가 발행하는 잔액 변경 이벤트 — 이 서비스가 원장에 남기는 유일한 입력이다.
 *
 * Step 5a 전에는 {@code transfer.credited}를 듣고 송금 한 건을 두 줄로 적었다. 그러면
 * 송금이 아닌 잔액 변경(입출금 API, 보상 환불)이 원장에 남지 않아 "원장 합 = 잔액"이 성립하지
 * 않았고, 정합성 대사를 할 수가 없었다. 이제 잔액이 움직이는 모든 경로가 이 이벤트를 낸다.
 *
 * 서비스 간 공유 모듈을 두지 않기로 했으므로 account 서비스도 같은 계약을 각자 정의한다.
