 ...
 *
 * 여기엔 Outbox를 두지 않았다. Outbox는 "DB 커밋"과 "이벤트 발행"을 원자적으로 묶는
 * 장치인데, 이 서비스는 그 둘이 어긋나도 스스로 복구된다.
 *   - 기록은 성공했는데 발행이 실패하면 → 오프셋이 커밋되지 않아 이벤트가 재전송되고,
 *       원장 기록이 멱등하므로 다시 기록해도 그대로다. 그리고 다시 발행한다.
 *   - 기록이 실패하면 → 발행 자체를 하지 않는다.
 * 즉 멱등한 쓰기 + 발행 후 ack 조합이면 같은 보장을 얻는다.
 * (게다가 MongoDB는 단일 노드에서 다중 문서 트랜잭션을 쓸 수 없어, 있어도 원자적이지 않다.)
 *
 * {@code block()}을 쓰는 이유: Kafka 리스너는 전용 스레드에서 돌기 때문에 막아도 이벤트 루프를
 * 굶기지 않는다. 오히려 여기서 끝날 때까지 기다려야 실패 시 오프셋이 커밋되지 않는다.
 */
@Component
