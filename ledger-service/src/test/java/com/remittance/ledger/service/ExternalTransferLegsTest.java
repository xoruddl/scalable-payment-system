...
/**
 * 상대 은행으로 나간 송금도 원장이 두 다리로 세는가 (Phase 6.5).
 *
 * 왜 이 테스트가 있어야 하나 ★
 * 이건 기능 검증이 아니라 아키텍처 결정의 전제 검증이다.
 *
 * 외부 송금은 받는 쪽이 우리 계좌가 아니라서 원장에 한 다리밖에 안 남는다. 그런데 원장은
 * 두 다리가 모여야 종결 신호({@code transfer.ledger-recorded})를 내고, 그게 없으면
 * 송금이 영영 COMPLETED가 되지 않는다.
 *
 * 그래서 상대 은행마다 정산 계좌를 두고 그쪽으로 적기로 했다. 그 선택의 근거가
 * "그러면 원장·대사 로직을 하나도 안 고쳐도 된다"였는데, 그건 코드를 읽고 내린 추론이었다.
 * 추론 위에 다음 Step을 쌓기 전에 여기서 못 박는다.
 *
 * 핵심은 {@code isTransferFullyRecorded}가 계좌가 아니라 reason을 센다는 점이다.
