...
/**
 * 대사 결과를 알리는 경로 (Phase 5 Step 2).
 *
 * Phase 2에서 대사는 만들었지만 결과를 아는 방법이 API를 직접 열어보는 것뿐이었다.
 * 아무도 열어보지 않으면 어긋남은 DB에만 조용히 쌓인다. 대사의 값어치는 찾아내는 데 있는 게
 * 아니라 사람에게 닿는 데 있다.
 *
 * 가장 중요한 지표는 발견 건수가 아니라 경과 시간이다
 * {@code last.run.age.seconds}가 이 클래스의 핵심이다. 발견 건수만 보면
 * "어긋난 게 없었다"와 "대사가 아예 안 돌았다"가 똑같이 0이다.
 ...
 * 5초마다 조회가 나가는데, 정작 DB가 흔들릴 때 지표까지 함께 잃는다.
...
public class ReconciliationMetrics {
	...
	/**
	 * 한 번도 돌지 않았으면 NaN을 낸다. 0을 내면 "방금 돌았다"는 거짓말이 되고,
