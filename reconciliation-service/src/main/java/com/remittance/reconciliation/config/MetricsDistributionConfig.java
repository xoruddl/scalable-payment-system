...
/**
 * p95·p99를 계산할 수 있도록 히스토그램 버킷을 붙인다 (Phase 5 Step 2).
 *
 * 기본값으로는 {@code _count}와 {@code _sum}만 나온다. 그 둘로 구할 수 있는 건 평균뿐인데,
 * 평균은 꼬리를 감춘다 — 이 프로젝트가 보려는 건 정확히 그 꼬리다.
 * 버킷이 없으면 Grafana의 {@code histogram_quantile()}은 오류를 내지 않고
 * 조용히 빈 패널을 낸다. 화면을 열어보기 전까지 아무도 모른다.
 *
 * 왜 YAML이 아니라 코드인가
 * 처음에는 {@code application.yml}의 {@code management.metrics.distribution}에 적었다.
 * 그런데 {@code src/test/resources/application.yml}이 같은 이름이라 운영 설정을 통째로 가린다.
 ...
 * 충돌해 요청마다 {@code InvalidConfigurationException}이 터진 적이 있다.
 * 기동은 멀쩡했고 헬스체크만 500을 냈다.
 *
 * 여섯 서비스가 같은 정책을 각자 정의한다 (상대 은행은 Kotlin으로).
...
public class MetricsDistributionConfig {
	...
	/**
	 * 분산 락을 쥐고 있던 시간 (Phase 6 Step 1). 대기보다 훨씬 짧은 쪽을 촘촘히 본다 —
	 * 임계 구역이 JPA 트랜잭션 하나라 밀리초 단위이고, 여기가 곧 한 계좌의 처리량 상한이다.
