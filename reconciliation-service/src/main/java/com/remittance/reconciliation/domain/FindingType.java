...
public enum FindingType {
	...
	/**
	 * 계좌 잔액과 원장 합이 다르다. 돈이 실제로 어긋난 것이라 가장 무겁다.
	...
	/**
	 * 상대 은행에 보냈는데 오래도록 결과를 모른다 (Phase 6.5).
	 *
	 * 우리도 상대도 모르는 돈이다. 확인 루프가 계속 물어보고 있지만 상대가 답을 못 주면
	 * 기계는 여기서 더 나아갈 수 없다 — 재시도하면 이중 입금이고, 실패로 처리하면 돈이 사라진다.
	 * 사람이 상대 은행에 연락해야 하는 유일한 종류라 따로 센다.
	 *
	 * 회로나 격벽에 막혀 못 보낸 건은 여기 들어오지 않는다. 돈이 나가지 않았으므로
