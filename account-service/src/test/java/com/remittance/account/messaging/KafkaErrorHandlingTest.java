 ...
 *
 * 확인하려는 계약은 둘이다.
 *   - 일시적인 실패는 재시도되어 결국 처리된다 — 한 번 삐끗했다고 송금이 멈추면 안 된다.
 *   - 끝내 실패한 메시지는 사라지지 않고 DLT에 남는다 — 돈이 걸린 이벤트라 유실이 곧 사고다.
 *
 * 이 테스트만 컨슈머 그룹을 따로 쓴다. 다른 테스트 컨텍스트와 그룹을 공유하면 메시지가
 * 다른 컨텍스트의 리스너에게 갈 수 있어, "이 컨텍스트의 스파이가 몇 번 불렸나"를 셀 수 없다.
 */
...
class KafkaErrorHandlingTest extends AbstractIntegrationTest {
	 ...
	 *
	 * 아직 한 건도 없으면 미터 자체가 없다 — 카운터는 처음 증가할 때 만들어진다.
	...
	/**
	 * 낙관적 락 충돌처럼 다시 하면 될 수도 있는 실패다. 여기서 포기하면 송금이 멈춘다.
	 */
	@Test
	 ...
	 * 고객 돈이 사라진 채로 아무 기록도 남지 않는다. DLT에 남아야 사람이 찾아낼 수 있다.
	 *
	 * 이런 실패는 재시도 없이 곧바로 DLT로 가야 한다. 결과가 달라질 리 없는데 백오프를
