...
/**
 * Phase 6 Step 2 — 리스너가 정말로 파티션 수만큼 스레드를 쓰는지 본다.
 *
 * 이 검증을 굳이 두는 이유는 두 가지다.
 *
 * 첫째, 틀려도 아무 일이 안 일어난다. 스레드가 1개로 돌아가도 기능은 전부 정상이고
 * 예외도 로그도 없다. 느려질 뿐이라, 부하를 걸어 보기 전에는 알 방법이 없다.
 * 실제로 2026-08-22 측정에서 {@code transfer.requested}에 lag 13,800이 쌓인 뒤에야
 * 스레드가 1개라는 걸 알았다.
 *
 * 둘째, 설정이 조용히 무력화되기 쉽다. {@code spring.kafka.listener.concurrency}로 줬다면
 * 테스트용 {@code application.yml}이 운영 설정을 가려 테스트는 스레드 1개로 돌았을 것이다
 * ({@code MetricsDistributionConfig}에서 이미 한 번 밟은 함정이다).
 *
 * {@code getConcurrency()}가 아니라 실제로 만들어진 자식 컨테이너 수를 센다.
...
class TransferEventConsumerConcurrencyTest extends AbstractIntegrationTest {
	 ...
	 *
	 * 이렇게 두면 검증하는 명제가 "지금 3이 맞나"에서
	 * "스레드가 파티션을 남김없이 쓰고 있나"로 바뀐다 — 그게 원래 알고 싶던 것이다.
	 */
	@Test
	...
	/**
	 * 스레드는 파티션을 하나씩 맡는다. 파티션보다 많이 띄우면 남는 스레드는 할당받을 게 없어
	 * 놀기만 한다 — 처리량은 그대로인데 컨슈머만 늘어 리밸런싱 비용만 커진다.
	 *
	 * 그래서 이건 "지금 3이 맞나"가 아니라 더 올리려면 파티션부터 늘려야 한다는
