...
/**
 * Phase 2 Step 4b — 보상이 실제로 한 바퀴 도는지.
 *
 * Step 0의 재현 테스트 #4가 지적한 문제다. 출금은 성공했는데 입금이 실패하면 돈이 공중에 뜬 채로
 * 멈춘다 — 출금 계좌에서는 빠졌고, 입금 계좌에는 들어가지 않았고, 아무도 되돌리지 않는다.
 *
 * {@link TransferSagaServiceTest}는 각 단계를 하나씩 직접 불러 검증한다. 여기서는
 * 진짜 Kafka와 진짜 Outbox 릴레이를 통해 이벤트가 스스로 다음을 부르는지를 본다.
 * 보상 흐름은 세 번의 배달을 거치므로(credit-failed 발행 → 수신 → debit-reversed 발행),
 * 배선이 한 군데만 어긋나도 조용히 멈춘다 — 단계별 테스트로는 그걸 못 잡는다.
 *
 * 릴레이를 켜는 유일한 테스트라 컨텍스트가 따로 뜬다. 느린 대신, 이 서비스 안에서
