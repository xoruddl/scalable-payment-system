...
/**
 * 발행이 끝난 Outbox 행을 보관 기간이 지나면 지운다 (Phase 4).
 *
 * 왜 이 테스트가 있나 — 실제로 당했다
 * 릴레이는 미발행 건만 읽고 나머지는 그냥 둔다. 그래서 사흘 만에
 * account 240만 건(1,105MB) · transfer 120만 건(551MB)이 쌓였고,
 * 같은 100 TPS에서 종결 p99가 4,673 → 5,044ms로 SLO를 넘었다.
 * 지우고 다시 재니 4,639ms로 돌아왔다.
 *
 * ★ 이 테스트가 한 번 거짓말을 했다
 * 처음에는 테스트 메서드에 {@code @Transactional}을 붙이고 리포지토리를 직접 불렀다.
 * green이었는데 홈서버에서는 매 주기 {@code TransactionRequiredException}으로 죽고 있었다 —
 * 운영 코드에는 트랜잭션이 없었고, 테스트가 그걸 대신 만들어주고 있었기 때문이다.
 *
 * 그래서 지금은 트랜잭션을 테스트가 주지 않는다. 운영에서 스케줄러가 부르는 것과 같이
 * {@link OutboxChunkDeleter}를 통해 부른다 — 그 빈이 자기 트랜잭션을 여는지가 여기서 갈린다.
 *
 * 거는 계약 — 셋째가 제일 중요하다
 *   1. 보관 기간이 지난 발행된 행은 지운다
 *   2. 기간이 안 지났으면 남긴다
 *   3. ★ 미발행 행은 아무리 오래돼도 안 지운다 — 지우면 그 이벤트가 영영 발행되지 않는다.
 *       돈이 한쪽만 움직인 채 끝난다
 */
@SpringBootTest
class OutboxRetentionTest extends AbstractIntegrationTest {
	...
	/**
	 * ⚠️ 테스트 준비도 쓰기라 트랜잭션이 필요하다. 그런데 테스트 클래스의 메서드에
	 * {@code @Transactional}을 붙여도 자기 클래스 내부 호출이라 프록시를 안 탄다 —
	 ...
	 * 오히려 느려진다 — 2026-08-29에 360만 건을 한 번에 지우고 곧바로 재봤다가
	...
	/**
	 * 발행이 끝났는데 아직 남아 있는 건수 — 이 지표가 없어서 못 봤다.
	 ...
	 *
	 * ⚠️ 기본키로 찾는다. {@code aggregate_id}는 MySQL에 binary(16)으로 들어가서
