...
/**
 * 상대 은행이 답하지 않았을 때 (Phase 6.5 Step 2b).
 *
 * 이 시스템에서 가장 불편한 상태다. 우리 고객 계좌는 이미 줄었는데 상대가 받았는지 모른다.
 * 실패로 처리하면 이미 나간 돈을 환불해 이중 지급이 되고,
 * 성공으로 처리하면 안 간 돈을 갔다고 하는 셈이다.
 *
 * 그래서 거는 계약은 셋이다.
 *   1. 답이 없으면 다시 보내지 않는다 — 재시도는 "안 갔다"를 전제로 하는데 그걸 모른다
 *   2. 대신 기록으로 남긴다 — 메모리에만 두면 재기동 한 번에 사라진다
 *   3. 결론은 조회로만 낸다 — 그쪽 장부에 무엇이 적혔는지가 유일한 근거다
 */
@SpringBootTest
class UnknownCreditTest extends AbstractIntegrationTest {
	 ...
	 *
	 * 이 테스트의 건만 본다. 컨테이너 DB를 여러 테스트가 공유하므로
	...
	/**
	 * 대사가 읽어갈 목록 — 사람을 부를 건만 들어가야 한다 (Phase 6.5).
	 *
	 * 못 보낸 건까지 넣으면 회로가 잘 동작할 때마다 사람을 부르게 되고,
	 * 아직 스스로 풀릴 시간이 남은 건까지 넣으면 진짜 신호가 묻힌다.
	 */
	@Test
