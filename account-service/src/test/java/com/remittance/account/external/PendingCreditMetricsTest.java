...
/**
 * "보냈는데 모른다"와 "보내지도 못했다"를 한 숫자로 세면 안 된다 (Phase 6.5).
 *
 * 왜 이 테스트가 있나 — 실제로 부풀려 보고했다
 * 2026-08-28 재측정에서 회로가 열렸을 때 {@code credit.unknown} 게이지가 320이었다.
 * 그런데 진짜로 모르는 건은 1건이고 나머지 319건은 회로에 막혀 아예 보내지 않은 것이었다.
 * 게이지가 표의 행 수를 셌기 때문이다.
 *
 * 차단된 건은 돈이 나갈 수가 없다. 사고가 아니라 밀린 일이다. 둘을 섞으면
