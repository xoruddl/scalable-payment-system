...
/**
 * "답을 못 받았다"를 예외 타입 하나로 판별하면 안 된다 (Phase 6.5 Step 2b).
 *
 * 왜 이 테스트가 있나 — 실제로 당했다
 * 처음에는 {@link ResourceAccessException}만 잡았다. 단위 테스트도 통과했고
 * 통합 테스트도 통과했다 — 거기서는 클라이언트가 목이었기 때문이다.
 *
 * 홈서버에서 진짜로 돌려보니 타임아웃이 그 타입으로 오지 않았다.
 * 응답 헤더는 받았는데 본문을 읽다가 끊겨서
 * {@code RestClientException: Error while extracting response ...}로 왔다.
 *
 * 그 결과 "모르는 상태"가 만들어지지 않고 메시지가 DLT로 죽었다.
 * 돈은 나갔을 수 있는데 아무도 확인하지 않는, 이 Phase가 없애려던 바로 그 상태다.
 *
 * 그래서 타입이 아니라 원인 사슬을 본다. 여기 적은 세 모양은
 * 실제로 관측된 것과 그 이웃들이다.
 */
