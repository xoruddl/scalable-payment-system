 ...
 * 결이 다르다 — 저쪽은 "흐름의 다음 단계"를 부르고, 이쪽은 일어난 사실을 기록으로 남기라고 알린다.
 *
 * 파티션 키는 계좌 ID다. 한 계좌의 잔액 변경은 순서대로 소비되어야 잔액 추이가 뒤섞이지 않는다.
 * (Saga 이벤트는 송금 ID를 키로 쓴다 — 목적이 다르면 키도 다르다.)
 */
public final class AccountEvents {

	/**
	 * 잔액이 바뀌었다. 원장은 이 이벤트만 보고 기록한다.
	 *
	 * Phase 2 Step 5a 전에는 원장이 {@code transfer.credited}만 듣고 송금 두 줄을 적었다.
	 * 그러면 송금이 아닌 잔액 변경(입출금 API, 보상 환불)이 원장에 남지 않아
	 ...
	 *
	 * @param entryId     이 변경 하나를 가리키는 ID. Outbox에 기록할 때 한 번 만들어 고정되므로,
	 *                    재전송이 와도 같은 값이다. 원장은 이걸 문서 ID로 삼아 멱등하게 기록한다.
	 * @param transferId  송금 때문에 움직인 경우에만 채워진다. 입출금 API로 움직였으면 {@code null}.
	 * @param balanceAfter 변경 후 잔액. 원장만 보고도 잔액 추이를 재구성할 수 있어야 한다.
	 */
	...
	public enum BalanceChangeReason {
		...
		WITHDRAWAL,
		/**
		 * 원장을 도입하기 전에 이미 쌓여 있던 잔액을 한 줄로 이월한 것.
		 *
		 * 다른 이유들과 성격이 다르다 — 이 항목은 잔액을 움직이지 않는다. 과거에 실제로
		 * 일어났지만 원장에 남지 않은 변경들을 뭉뚱그려 적어, 지금 잔액과 원장 합을 맞추는 것이다.
		 * (Step 5a 이전에 만들어진 계좌들이 영원히 BALANCE_MISMATCH로 잡히던 문제)
		 *
		 * 계좌당 한 번만 남는다. 두 번 심으면 그 즉시 원장이 잔액보다 커진다.
		 */
		OPENING_BALANCE
	}
}
