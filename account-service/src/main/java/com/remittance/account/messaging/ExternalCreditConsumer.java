...
/**
 * 상대 은행으로 나가는 입금만 받는다 — 별도 컨슈머 그룹, 별도 스레드 (Phase 6.5).
 *
 * 왜 나눴나 — 숫자가 시켰다
 * 같은 리스너에서 내부 입금과 외부 호출을 다 하니 느린 상대가 우리 내부 송금을 묶었다.
 *
 ...
 * | 리스너 분리 | ← 이 커밋이 재려는 값 |
 *
 * 격벽은 같은 풀을 나눠 쓰면서 덜 뺏기는 방법이다. 거기까지가 한계였다 —
 ...
 * 대신 같은 토픽을 두 그룹이 각자 읽고 자기 몫만 처리한다.
 * 메시지를 두 번 읽는 것이 대가인데, 남의 몫은 JSON 한 번 읽고 버리는 것뿐이라 싸다.
 *
 * ⚠️ 처음 뜰 때 한 번 밀린 것을 다 읽는다
 * 새 그룹이라 {@code auto-offset-reset: earliest}로 토픽 처음부터 읽는다.
 * 지금 쌓인 50만 건을 훑고 지나간다 — 대부분 내부 송금이라 걸러지지만 시간은 걸린다.
 * {@code latest}로 두면 배포와 기동 사이에 발행된 외부 송금이 통째로 사라지므로
...
public class ExternalCreditConsumer {
	...
	/**
	 * 외부 호출 전용 스레드 수. 이 값이 곧 "상대에게 동시에 몇 개까지 보내나"이고,
	 * 동시에 이 서비스가 외부 때문에 잃을 수 있는 최대 스레드 수다.
	...
	/** 내부 리스너와 다른 그룹이라야 각자 자기 스레드를 갖는다. */
