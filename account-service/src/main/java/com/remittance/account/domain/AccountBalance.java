...
/**
 * 계좌 하나와 그 계좌 잔액의 조각들을 함께 다루는 단위. 잔액을 바꾸는 모든 일이 여기를 거친다.
 *
 * 엔티티가 아니라 한 트랜잭션 동안만 사는 묶음이다. 계좌와 조각이 서로 다른 테이블에
 * 있는데 업무 규칙(잔액이 모자라면 못 뺀다, 정지된 계좌는 못 쓴다)은 둘을 함께 봐야 성립한다.
 * 그 규칙이 서비스 코드로 흩어지지 않게 여기 모은다.
 *
 * 전에는 이 자리가 {@code Account}였다. 잔액이 계좌 행에 있었기 때문이다.
 * 옮기고 나서 {@code Account}에 남은 것은 계좌가 쓸 수 있는 상태인가뿐이다.
 */
public final class AccountBalance {
	...
	/** 읽지 않은 조각들의 합. 입금은 조각 하나만 읽으므로 나머지가 여기 담긴다. */
	...
	/**
	 * 조각을 전부 읽어온 경우. 출금과 조회가 이걸 쓴다 — 합을 알아야 하기 때문이다.
	 */
	...
	/**
	 * 조각 하나만 읽어온 경우. 입금이 이걸 쓴다.
	 *
	 * 여기가 쪼갠 이득이 나오는 지점이다. 조각 하나만 읽고 그 행만 바꾸므로,
	 * 같은 계좌의 다른 입금이 다른 조각을 만지는 한 서로 기다리지 않는다.
	 *
	 * @param unreadTotal 읽지 않은 조각들의 합. 분개장에 남길 변경 후 잔액을 만드는 데 쓴다.
	 *                    조각이 하나뿐인 계좌면 0이다.
	 */
	 ...
	 *
	 * ⚠️ 조각 하나만 읽어온 경우({@link #onlyShard}) 나머지 조각의 합은 읽은 시점의 값이다.
	 * 그 사이 다른 입금이 커밋됐으면 여기 반영되지 않는다. 쪼갠 계좌의 "변경 후 잔액"은
	 * 근사치라는 뜻이고, 이것도 쪼개는 대가다. 정합성 대사는 이 값을 쓰지 않는다 —
	...
	/**
	 * 넣는다. 읽어온 첫 조각에만 더한다 — 어느 조각에 넣을지는 읽어올 때 이미 정해졌다.
	 */
	...
	/**
	 * 뺀다. 합을 보고 조각들에서 훑어 뺀다.
	 *
	 * 입금과 달리 조각 하나만 볼 수 없다. 그 조각에 없어도 옆 조각에 있으면 계좌에는 있는
	 * 돈이라, 조각 하나만 보고 거절하면 잔액이 있는데 실패하는 일이 생긴다.
	 * 그래서 출금은 계좌 전체를 본다 — 쪼갠 이득이 출금에는 없다는 뜻이고, 이게 대가다.
	 *
	 * 앞 조각부터 채워 뺀다. 순서를 고정하는 이유는 같은 계좌에 출금이 둘 동시에 오면
