...
/**
 * 한 계좌 잔액의 조각. 계좌의 진짜 잔액은 이 조각들의 합이다.
 *
 * 왜 쪼개나 — 잔액이 계좌 행 하나에 있으면 그 계좌의 모든 입금이 그 행 하나에 줄을 선다.
 * 2026-08-23 측정에서 락 보유 p50이 38ms였고, 그건 한 계좌 초당 26건이라는 뜻이다.
 * 서버를 늘려도 안 변하는 숫자다. 조각이 N개면 서로 다른 행을 만지므로 그만큼 나란히 간다.
 *
 * {@link Version}이 계좌가 아니라 여기 붙어 있는 것이 쪼개는 의미의 절반이다.
 * 계좌에 붙어 있으면 어느 조각을 만졌든 같은 행의 버전을 올려 결국 다시 충돌한다.
 *
 * 대부분의 계좌는 조각이 하나다. 경합이 없는 계좌를 쪼개면 조회할 때마다 합산만 늘어 손해다.
 */
@Entity
...
public class AccountBalanceShard {
	...
	/**
	 * 이 조각에서 뺀다. 조각 하나가 마이너스가 되면 안 된다 — 합이 맞아도 개별 조각이
