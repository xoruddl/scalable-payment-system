 ...
 *
 * 왜 {@link OutboxRelay}에서 분리했나
 * 적체가 있을 때 다음 폴링을 기다리지 않고 이어서 비우려면 배치 하나가 트랜잭션 하나여야 한다.
 ...
 * 100건이면 브로커와 100번 왕복한다. 2026-08-22 baseline에서 종결 처리량이
 * 초당 19건에 묶여 있던 원인이 여기다.
 *
 * 지금은 전부 보내고 마지막에 한 번 기다린다. 프로듀서가 알아서 묶어 보내므로
 * 왕복이 사실상 한 번으로 줄어든다.
 *
 * 순서는 어떻게 지키나
 * 애그리거트 ID를 키로 쓰므로 같은 송금의 이벤트는 같은 파티션으로 간다.
 * 프로듀서가 {@code acks=all} + {@code enable.idempotence=true}라, 재시도가 일어나도
 * 파티션 안에서의 순서가 뒤바뀌지 않는다 (어긋나면 브로커가 거절한다).
 *
 * 그래도 남는 창이 있다 — 앞의 것이 끝내 실패하면 뒤의 것은 이미 보내진 뒤다.
 * 그때는 실패 지점부터 마킹하지 않고 다음 폴링에서 다시 보낸다. 즉 중복은 생길 수 있고
 * 순서는 어긋날 수 있다. 둘 다 이 시스템이 이미 견디도록 만들어져 있다 —
...
public class OutboxBatchPublisher {
	...
	/**
	 * ★ 왜 READ COMMITTED인가 — SKIP LOCKED만으로는 부족했다.
	 *
	 * MySQL 기본값인 REPEATABLE READ에서는 {@code FOR UPDATE}가 스캔한 인덱스 구간에
	 * 갭 락까지 겁니다. 릴레이 셋이 동시에 돌자 서로의 갭을 물고
	 * {@code Deadlock found when trying to get lock}으로 죽었다 — {@code SKIP LOCKED}는
	 * 잠긴 행을 건너뛸 뿐 갭 락을 없애주지 않기 때문이다.
	 *
	 * READ COMMITTED는 갭 락을 걸지 않고 행 락만 잡는다. 큐를 나눠 갖는 이 패턴에
	 * 맞는 격리 수준이고, 여기서 잃는 것도 없다 — 이 트랜잭션은 한 번 읽고 그 행만 갱신하므로
	 * 반복 읽기의 일관성이 필요 없다.
	 *
	 * 이 결함은 인스턴스가 둘 이상일 때만 나타난다. 테스트에서 잡힌 것은 운이 좋았다 —
	 * 릴레이를 켜둔 다른 테스트의 Spring 컨텍스트가 캐시되어 세 번째 릴레이로 같이 돌았다.
	 *
	 * @return 실제로 발행하고 마킹한 건수. {@code batchSize}보다 적으면 더 비울 게 없거나
	 *         중간에 실패한 것이므로, 부르는 쪽은 이번 주기를 여기서 끝내면 된다.
	 */
