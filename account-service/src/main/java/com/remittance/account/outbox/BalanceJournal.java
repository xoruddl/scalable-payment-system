...
/**
 * 잔액 변경을 분개장에 남기라고 알리는 이벤트를 Outbox에 적는다.
 *
 * 잔액이 바뀌는 경로는 둘이다 — 입출금 API와 송금 Saga. 둘 다 여기를 거쳐야 한다.
 * 한 경로라도 빠지면 "원장 합 = 잔액"이 깨지고, 그 순간 정합성 대사는 의미가 없어진다.
 * (Step 5a 전에는 송금만 원장에 남아서 실제로 성립하지 않았다.)
 *
 * 이 메서드는 반드시 잔액을 바꾼 트랜잭션 안에서 호출되어야 한다. 밖에서 부르면
