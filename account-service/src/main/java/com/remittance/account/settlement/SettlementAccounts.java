...
/**
 * 상대 은행마다 하나씩 있는 정산 계좌를 찾아준다 (Phase 6.5).
 *
 * 왜 있는가
 * 외부로 나가는 송금은 받는 쪽이 우리 계좌가 아니다. 그런데 원장은 송금 한 건에
 * 두 다리가 모여야 종결로 본다. 상대 계좌를 우리 원장에 적을 수는 없으니,
 ...
 *
 * 메모리에 들고 있는 이유
 * 은행 수만큼만 있고 거의 안 바뀐다. 입금마다 계좌를 한 번 더 읽으면 커넥션을 한 번 더
...
public class SettlementAccounts {
	...
	/**
	 * 이 은행의 정산 계좌를 찾는다. 없으면 만든다.
	 *
	 * 미리 만들어두지 않는 이유는 은행이 늘어날 때 사람 손을 타지 않게 하기 위해서다.
