 ...
 *
 * 왜 락보다 먼저 정해야 하는가
 * 쪼갠 이득은 락이 조각별로 걸릴 때만 나온다. 계좌 하나에 락 하나면 조각을 아무리 나눠도
 * 그 락에서 다시 줄을 선다. 그런데 락 키를 만들려면 조각 번호가 필요하고,
 * 조각 번호를 알려면 계좌를 읽어야 한다 — 락 밖에서 읽어야 한다는 뜻이다.
 *
 * 그래서 미리 들고 있는다
 * 입금마다 계좌를 한 번 더 읽으면 커넥션을 한 번 더 잡는다. Phase 6 Step 2에서 첫 병목이
 * 정확히 커넥션 대기였으므로, 그걸 되살릴 이유가 없다.
 *
 * 쪼갠 계좌만 담는다. 나머지는 전부 1이라 담을 이유가 없고, 이 맵이 계좌 수만큼
 * 자라지도 않는다 — 핫 계좌는 몇 개뿐이다.
 *
 * 늦게 알아도 안전한 이유
 * 다른 인스턴스가 계좌를 쪼개면 여기 반영되기까지 최대 {@link #REFRESH_MS}가 걸린다.
 * 그동안은 예전 조각 수로 고른다. 조각은 늘리기만 하고 줄일 수 없으므로
 * (Account#widenShards), 예전 수로 고른 번호는 반드시 존재한다.
...
public class ShardRouter implements InitializingBean {
	 ...
	 * 어느 조각에 넣을지 고른다. 무작위다.
	 *
	 * 순번을 돌리면(round-robin) 인스턴스마다 자기 순번을 갖게 되어, 인스턴스가 여럿이면
	 * 같은 조각을 동시에 고르는 일이 오히려 잦아진다. 송금 ID로 해시하는 방법도 있지만,
	 * 그러면 같은 송금의 재시도가 늘 같은 조각으로 몰려 경합이 난 조각을 계속 다시 두드린다.
	 */
