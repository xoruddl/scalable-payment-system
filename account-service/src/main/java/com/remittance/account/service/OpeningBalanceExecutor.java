...
/**
 * 개시 잔액 이월 한 건을 하나의 트랜잭션으로 처리한다 — 이월 표시와 분개 기록이 함께 커밋되어야 한다.
 *
 * 표시만 남고 분개가 없으면 그 계좌는 영원히 어긋난 채 다시 이월할 수도 없게 되고,
 * 분개만 남고 표시가 없으면 다음 실행이 또 심어 원장이 잔액보다 커진다.
 *
 * {@link BalanceMutationExecutor}와 짝이지만 잔액을 바꾸지 않는다는 점이 다르다.
 */
@Component
