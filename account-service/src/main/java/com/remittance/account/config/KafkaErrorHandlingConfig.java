...
/**
 * 컨슈머가 실패했을 때 무엇을 할지 정한다 — 몇 번 다시 해보고, 그래도 안 되면 어디에 둘지.
 *
 * 이걸 두지 않으면 spring-kafka 기본값이 적용되는데, 그 기본값은
 * 지연 없이 10번 재시도하고 로그만 남긴 뒤 오프셋을 커밋한다. 즉 메시지가 조용히 사라진다.
 * 잠깐 DB가 끊긴 것뿐이었어도 1초 안에 10번을 몰아 시도하고 포기하므로, 회복될 틈도 없다.
 *
 * 바꾼 것은 두 가지다.
 *   - 지수 백오프: 1초 → 2초 → 4초. 일시적인 장애가 회복될 시간을 준다.
 *   - DLT: 끝내 실패하면 {@code <원래 토픽>.DLT}로 보낸다. 사라지지 않고 남아야
 *       사람이 보고 처리할 수 있다 — 돈이 걸린 이벤트라 유실이 곧 사고다.
 *
 * 재시도가 의미 있으려면 컨슈머가 멱등해야 한다. 이 서비스는 처리 흔적(processed_events)으로
 * 그걸 보장하므로, 같은 이벤트를 몇 번 다시 처리해도 잔액은 한 번만 움직인다.
 *
 * 경합은 포기하지 않는다 (Phase 6 Step 1)
 * 2026-08-23 핫 계좌 측정에서 송금 9건이 영영 갇혔다. 상태가 {@code DEBIT_COMPLETED} —
 * 출금은 됐는데 입금이 안 된 채로 멈춘 것이고, 이건 이 시스템에서 가장 나쁜 결말이다.
 *
 * 원인은 여기였다. 락을 3초 안에 못 잡으면 {@link LockAcquisitionException}이 올라오는데,
 * 그게 여느 실패와 똑같이 취급돼 재시도 세 번 만에 DLT로 죽었다.
 *
 * 그런데 락을 못 잡은 것은 "지금 붐빈다"는 뜻이지 처리할 수 없는 메시지가 아니다.
 * 잠시 뒤에 다시 하면 된다. 여기에 횟수 제한을 두면 붐빈다는 이유로 돈을 버리는 셈이다.
 * 그래서 경합 계열 예외는 횟수 제한 없이 재시도한다.
 *
...
public class KafkaErrorHandlingConfig {
	 ...
	 *
	 * {@code static}으로 둔 이유는 테스트에서 직접 부를 수 있게 하기 위해서다.
	 * "무한 재시도가 실제로 일어나는지"는 통합 테스트로 재현하기 어렵지만,
	 * 어떤 예외에 어떤 정책이 붙는지는 여기서 정확히 확인할 수 있다.
	 */
	 ...
	 * 경합 때문에 실패한 것인가.
	 *
	 * 원인 사슬을 따라간다. 리스너에서 난 예외는 spring-kafka가
	...
	/**
	 * DLT로 보내면서 흔적을 남긴다 (Phase 5 Step 2).
	 *
	 * 2026-08-22 e2e에서 확인한 것: 본문이 깨진 메시지를 넣었더니 설계대로 DLT에 쌓이고
	 * 파티션도 막히지 않았는데, WARN·ERROR 로그가 한 줄도 남지 않았다.
	 * {@code DeadLetterPublishingRecoverer}는 성공적으로 보낸 사실을 알리지 않는다.
	 * 메시지 하나가 죽어서 사람 손을 기다리는 중인데 아무도 모르는 상태가 된다.
	 *
	 * 그래서 두 가지를 붙였다.
	 *   - WARN 로그 — 어느 토픽·파티션·오프셋의 무엇이 왜 죽었는지. 발행보다 먼저
	 *       남긴다. DLT 발행 자체가 실패해도 최소한 무엇을 잃었는지는 알아야 한다.
	 *   - {@code remittance.kafka.dlt.published} 카운터 — 토픽별. 로그는 사람이 볼 때만
	 *       보이지만 이 값은 그래프에서 튄다. 토픽 수가 고정이라 라벨이 늘어날 걱정도 없다.
	 *
	 * 파티션을 {@code -1}로 넘기는 게 핵심이다. 기본 동작은 원래 메시지와 같은 번호의 파티션으로
