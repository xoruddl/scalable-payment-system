 ...
 *
 * 공개와 내부를 반드시 가른다 ★
 * `/internal/*`은 대사·운영 도구가 쓰는 문이라 Gateway 밖으로 나가면 안 된다.
 * 한 문서에 섞어두면 Gateway가 그대로 노출할 때 남의 계좌 잔액을 고치는 문이 공개 API로
 * 보이게 된다. 그래서 그룹을 둘로 나눈다.
 *
 *   경로                           무엇                 누가 본다
 *   {@code /v3/api-docs/public}    외부에 노출할 것     Gateway가 모아서 낸다
 *   {@code /v3/api-docs/internal}  {@code /internal/*}  우리만
 *   {@code /v3/api-docs}           전부                 이 서비스에 직접 물었을 때
 *
 * 거르는 쪽이 기본이 아니다는 점이 중요하다 — 새 `/internal` 경로를 만들면
