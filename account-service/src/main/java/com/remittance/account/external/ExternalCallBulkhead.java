 ...
 *
 * 직접 만든 {@code Semaphore} 격벽에서 확인한 계약을 Resilience4j의 semaphore bulkhead로
 ...
 *
 * 거절 수는 우리가 센다 ★
 * {@link TaggedBulkheadMetrics}가 내는 것은 게이지 둘뿐이다 — 정원과 남은 자리.
 * 그런데 격벽이 일하고 있는지는 순간값이 아니라 얼마나 거절했나로 판정해왔다
 * (2026-08-27 실측의 "거절 1,662 &gt; 통과 1,273"이 정원을 늘린 근거였다).
 * 직접 만든 격벽에는 있던 그 카운터가 Resilience4j에는 없으므로 이벤트로 다시 만든다.
 *
 * 이름은 자체 구현 때와 같게 뒀다. 바꾸면 지나간 기록의 숫자와 이어지지 않는다.
