...
/**
 * 상대 은행에 보냈는데 답을 못 받은 입금 한 건.
 *
 * 이 행이 있다는 것은 돈이 나갔을 수도 있다는 뜻이다. 우리 고객 계좌는 이미 줄었는데
 * 상대가 받았는지 모른다 — 이 시스템에서 가장 불편한 상태이고, 그래서 반드시 기록으로 남긴다.
 * 메모리에만 두면 재기동 한 번에 사라지고, 그러면 아무도 모르는 채로 끝난다.
 *
 * 해소는 조회로만 한다. 다시 보내는 것은 답이 아니다 —
...
public class PendingExternalCredit {
	 ...
	 *
	 * 둘은 전혀 다른 상태다.
	 *   - {@code true} — 보냈는데 답이 없다. 돈이 나갔을 수 있다. 조회로 확인한다
	 *   - {@code false} — 격벽에 막혀 보내지도 못했다. 돈은 안 나갔다. 그냥 보내면 된다
	 *
	 ...
	 *
	 * 간격을 지수적으로 늘리되 상한을 둔다. 상대가 오래 아플 수 있으므로 계속 두드리면
	 * 그쪽 회복을 방해하고, 그렇다고 무한정 미루면 풀렸는데도 한참 모른 채로 남는다.
	 */
