...
/**
 * 원장 도입 이전 잔액을 이월하는 일회성 운영 작업용 API. Gateway로 노출되지 않는다.
 *
 * {@link InternalAccountController}(Transfer가 Saga에서 부르는 문)와 일부러 갈라놓았다 —
 * 이건 흐름의 일부가 아니라 사람이 한 번 돌리는 이행 작업이라, 누가 왜 부르는지가 다르다.
 *
 * 대사 서비스가 이걸 자동으로 부르지 않는다. 그러면 결국 대사가 남의 데이터를 고치는 셈이
