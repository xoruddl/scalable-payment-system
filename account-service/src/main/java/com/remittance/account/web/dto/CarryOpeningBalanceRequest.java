...
/**
 * 개시 잔액 이월 요청. 이월할 금액이 아니라 관측한 두 값을 보낸다 —
 * 금액은 계좌 서비스가 직접 빼서 정한다.
 *
 * 금액을 그대로 받으면 호출자가 부르는 대로 원장에 줄이 생긴다. 그러면 이 엔드포인트는
 * "남이 내 원장에 아무 숫자나 적을 수 있는 문"이 된다. 관측값을 받아 스냅샷이 아직 맞는지
 * 확인하고 스스로 계산해야, 데이터를 바꾸는 주체가 끝까지 주인으로 남는다.
 *
