...
> ```bash
> docker run --rm -i --cpus="4" -v "$PWD:/work" -w /work \
...
### ⚠️ `--cpus`를 꼭 붙이세요 — 안 붙이면 부하 생성기가 서비스를 굶깁니다

k6와 서비스가 **같은 노트북**에 있으면 CPU를 두고 싸웁니다. 그러면 "앱이 느린 것"과
"부하 생성기가 앱의 CPU를 뺏은 것"이 구분되지 않습니다.

2026-08-22 실측 — `read-heavy.js`를 같은 조건에서 제한만 다르게 두 번 돌린 결과입니다.

| | 제한 없음 | `--cpus="4"` |
|---|---|---|
| 처리량 | 211 req/s | **346 req/s** |
| p95 | 3,435 ms | **1,516 ms** |
| p99 | 4,324 ms | **1,608 ms** |

**부하 생성기를 굶겼더니 오히려 처리량이 64% 올랐습니다.** 그전엔 k6가 CPU를 가져가
정작 측정 대상이 일을 못 하고 있었습니다.

> `--cpuset-cpus`(특정 코어에 고정)는 macOS에서 기대만큼 동작하지 않습니다.
> 컨테이너는 Docker Desktop의 리눅스 VM 안에서 돌고, **그 VM의 vCPU를 다시 하이퍼바이저가
> 호스트 코어에 스케줄**하기 때문입니다. 총량 제한(`--cpus`)이 실제로 듣는 손잡이입니다.

### 그래도 이건 임시방편입니다

CPU만 나눌 뿐 메모리 대역폭·디스크·네트워크는 여전히 공유합니다.
제대로 하려면 **부하 생성기와 대상이 다른 머신에 있어야** 합니다. 조건이 둘입니다.

1. **자원을 공유하지 않을 것** — 같은 노트북이면 깨집니다
2. **네트워크가 짧을 것** — 원격 서버에 인터넷을 넘어 부하를 걸면 깨집니다

**로컬에서 원격 배포본을 때리는 건 1을 고치면서 2를 깹니다.** RTT가 접수 지연에 그대로
더해지고 VU 하나가 낼 수 있는 요청 수도 RTT에 묶여, 병목이 앱인지 인터넷인지 알 수 없게 됩니다.

| 방법 | 비용 | 얻는 것 |
|---|---|---|
| `--cpus`로 총량 제한 | 0 | 간섭 제한. **지금 쓰는 방법** |
| 남는 PC + 유선 LAN | 0 (있다면) | 진짜 분리 + 짧은 네트워크. 가성비 최고 |
| 같은 리전 클라우드 VM 2대 | 과금 | 정석. Phase 8(K8s)과 함께 |
| 관리형 부하 서비스 | 유료 | 편하지만 인터넷을 경유해 조건 2가 깨짐 |

Phase 8에서 K8s에 올라가면 k6를 **클러스터 안 Job으로, 부하 전용 노드에** 배치합니다.
(로컬 K8s는 결국 같은 노트북이라 이 문제가 그대로입니다.)
...
> ⚠️ **이 시나리오가 환경에 가장 민감합니다.** VU를 800개까지 올리므로 부하 생성기가
> CPU를 많이 먹습니다. `--cpus` 없이 돌리면 원장이 아니라 **노트북을 재게 됩니다.**
> 판별법: 지연이 큰데 `process_cpu_usage{service="ledger"}`가 낮으면 원장 탓이 아닙니다.
...
| **클라이언트가 먼저 죽음** | 앱은 한가한데 지연이 큼 | `--cpus="4"`로 k6를 제한하세요. 실측으로 처리량 211→346 req/s. 판별은 `process_cpu_usage{service="..."}`가 낮은지로 합니다 |
