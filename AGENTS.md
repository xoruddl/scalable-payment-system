...
| ~~`docs/openapi.yaml`~~ | **없앴습니다** (Phase 4). API 계약은 각 서비스가 `/v3/api-docs`로 스스로 냅니다 — 손으로 쓴 문서는 이미 코드와 어긋나 있었습니다. 공개용은 `/v3/api-docs/public`, 내부용은 `/v3/api-docs/internal` | 엔드포인트를 추가·변경할 때는 **문서를 고칠 필요가 없습니다** |
...
9. **알아야 할 것은 전부 저장소 문서에 적는다.** 에이전트의 개인 메모리나 세션 기록에 두지 마세요.
   사람도 다른 도구도 읽을 수 있어야 하고, 저장소와 함께 버전 관리되어야 합니다.
   자리는 위 "문서 지도"에 있습니다 — 규칙이면 `CONTRIBUTING.md`, 결정이면 `DECISIONS.md`,
   겪은 일이면 `PROGRESS.md`.
10. **커밋·PR 본문에 서사를 풀지 않는다.** 제목 한 줄로 무엇을 했는지 적고 본문은 불릿으로 씁니다.
    양식과 예시는 `docs/CONTRIBUTING.md`의 "커밋 컨벤션"·"PR 작성" 절에 있습니다.
    ⚠️ **2026-09-11 이전의 `git log`와 PR 본문은 본보기가 아닙니다** — 그 규칙이 생기기 전 것입니다.
...
>
> ⚠️ **재기동 직후 첫 실행은 버립니다.** JVM·커넥션 풀·컨슈머 리밸런스가 안 풀린 상태라
> 데워진 실행과 비교하면 없는 개선이 보입니다. 실제로 2026-08-24에 32.0초 vs 2.1초로
> "15.4배"라고 적을 뻔했고, 다시 재니 둘 다 2.1초였습니다. **A/B는 같은 온도에서 잽니다.**
>
> ⚠️ **DB 설정도 저장소와 같은지 확인합니다.** 실험용으로 손댄 MySQL 설정을 되돌리지 않은 채
> 잰 값을 결과로 적은 적이 있습니다 — 앱 커밋 해시는 확인했는데 DB는 아무도 확인하지 않았습니다.
> **서버 설정을 손으로 바꿨으면 되돌리는 것까지가 실험입니다.**
> `scripts/measure-hot-account.sh`가 이제 재기 전에 확인하고, 다르면 멈춥니다.
