 ...
 * 송금 상태 전이 규칙.
 *
 * 규칙의 핵심은 "앞으로만 간다"이다. Saga 이벤트는 단계마다 다른 토픽으로 오므로
 * 도착 순서가 보장되지 않는다. 그래서
 *   - 뒤 단계가 먼저 와도 건너뛰어서라도 적용한다 — 버리면 다시 오지 않아 송금이 멈춘다.
 *   - 지나간 단계가 다시 와도 되돌아가지 않는다 — 재전송에 대한 멱등성이다.
 *   - 되돌리는 중이거나 이미 닫혔으면 정상 흐름을 진행시키지 않는다.
 */
...
class TransferStateUpdaterTest {
	...
	/**
	 * 토픽이 다르면 도착 순서가 보장되지 않는다. 여기서 버리면 그 단계는 다시 오지 않아
	 ...
	 * 여기서 곧바로 FAILED로 닫으면 돈은 사라졌는데 실패로 끝난 송금이 된다.
	 */
	@Test
