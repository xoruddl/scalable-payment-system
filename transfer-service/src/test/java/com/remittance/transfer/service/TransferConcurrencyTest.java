 ...
 *
 * 세 서비스를 실제로 띄운 e2e에서 드러난 문제다. Saga 단계마다 토픽이 다르고
 * 토픽마다 리스너 스레드가 다르다. 여러 리스너가 같은 송금 행을 동시에
 * read-modify-write 할 수 있는데, {@code Transfer}에는 {@code @Version}이 없어
 * 마지막에 커밋한 쪽이 이긴다.
 *
 * e2e에서 실제로 이렇게 됐다 — {@code markFailed()}가 실행되어 {@code transfer.failed}까지
 * 발행된 송금의 행이, 뒤늦게 커밋된 출금 이벤트에 덮여 {@code DEBIT_COMPLETED}로 남았다.
 * 바깥에는 실패라고 알려놓고 자기 기록은 진행 중인 상태다.
 *
 * 경합을 운에 맡기면 테스트가 흔들리므로 순서를 강제한다. 출금 이벤트를 처리하는 스레드를
 * 읽은 뒤 · 커밋하기 전에 붙잡아두고, 그 사이 다른 이벤트가 읽고 쓰고 커밋하게 한다.
 *
 * 붙잡는 지점으로 {@code save}를 고른 이유가 있다. 조회를 가로채려면 Mockito의
 * {@code callRealMethod()}가 필요한데 Spring Data 리포지토리는 인터페이스라 쓸 수 없다
 * ("Cannot call abstract real method"). 반면 {@code save}는 실제로 부르지 않아도 된다 —
