 ...
 *
 * 세 서비스를 실제로 띄운 e2e에서 드러난 문제다. 이 서비스는 발행하는 토픽만
 * {@code NewTopic}으로 선언하고, 소비하는 토픽은 남이 만들어주기를 기대한다.
 * 그런데 컨슈머가 토픽이 생기기 전에 구독하면 브로커가 기본값(1파티션)으로 자동 생성해버린다.
 *
 * 그 뒤 발행하는 서비스가 3파티션으로 늘려도, 이미 붙은 컨슈머는 늘어난 파티션을 모른다
 ...
 *
 * 유실은 아니고 메타데이터가 갱신되면 회복되지만, 컨테이너·K8s로 매번 새 환경을 띄우는
...
class KafkaTopicPartitionTest extends AbstractIntegrationTest {
	 ...
	 *
	 * 기대값을 여기 적지 않고 운영 설정에서 그대로 읽는다. 따로 적어두면
	 * 같은 숫자가 두 곳에 있게 되고, 바꿀 때 한쪽만 고쳐 red를 보게 된다.
	 * 이 테스트가 볼 것은 "3인가"가 아니라 "선언한 대로 만들어졌는가"다.
	 */
	...
	/** 이 서비스가 소비만 하는 토픽들. 만드는 건 남이지만, 잘못 만들어지면 손해는 여기서 본다. */
	@ParameterizedTest
	@ValueSource(strings = {
