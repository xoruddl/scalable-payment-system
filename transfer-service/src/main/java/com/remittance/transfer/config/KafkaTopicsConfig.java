 ...
 * 나중에 늘려도 이미 쌓인 메시지의 키 분배가 달라져 순서 보장이 깨진다.
 *
...
public class KafkaTopicsConfig {
	...
	NewTopic transferRequestedTopic() {
		return topic(TransferEventType.REQUESTED.topic());
	}

	@Bean
	NewTopic transferCompletedTopic() {
		return topic(TransferEventType.COMPLETED.topic());
	}

	@Bean
	NewTopic transferFailedTopic() {
		return topic(TransferEventType.FAILED.topic());
	}
