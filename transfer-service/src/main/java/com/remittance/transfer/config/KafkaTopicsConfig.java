 ...
 * 컨슈머가 토픽이 생기기 전에 구독하면 브로커가 1파티션으로 자동 생성해버린다.
...
public class KafkaTopicsConfig {
	...
	/**
	 * 컨슈머 스레드 수의 상한이다 — 스레드는 파티션 수를 넘을 수 없다.
	 * 3 → 6으로 올렸다. 근거와 주의사항은 account-service의 같은 파일에 적어두었다
	 * (네 서비스가 같은 값을 선언해야 한다 — 어긋나면 큰 쪽이 이긴다).
	 *
	 * {@code private}이 아닌 이유: {@code KafkaTopicPartitionTest}가 이 값을 읽는다.
	 * 테스트가 기대값을 따로 적어두면 같은 숫자를 두 곳에 두는 것이라, 바꿀 때
