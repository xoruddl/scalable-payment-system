 ...
 *
 * 이 서비스가 이벤트를 잃으면 송금이 실제 상태와 다른 상태로 남는다 — 돈은 이미 움직였는데
 * 조회하면 PENDING인 식이다. 그래서 끝내 처리하지 못한 메시지는 DLT에 남겨 사람이 보게 한다.
 *
 * 세 서비스가 같은 정책을 각자 정의한다. 공유 모듈을 두지 않기로 했으므로,
 * 바꿀 때는 세 곳을 함께 확인해야 한다 (이벤트 계약과 같은 규칙).
 */
@Configuration
public class KafkaErrorHandlingConfig {
	...
	DefaultErrorHandler kafkaErrorHandler(KafkaTemplate<String, String> kafkaTemplate, MeterRegistry meterRegistry) {
		...
		errorHandler.addNotRetryableExceptions(
				// 존재하지 않는 송금의 이벤트는 다시 받아도 여전히 존재하지 않는다.
				// 정상 경로라면 생길 수 없는 일이라, 재시도보다 사람이 봐야 할 사건에 가깝다.
