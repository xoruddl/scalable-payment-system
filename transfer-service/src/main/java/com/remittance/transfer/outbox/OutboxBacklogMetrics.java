...
/**
 * Outbox에 아직 발행되지 않고 쌓인 이벤트 수를 내보낸다 (Phase 5 Step 2).
 *
 * 이 숫자가 없으면 릴레이가 부하를 못 따라가는 순간을 바깥에서 볼 방법이 없다.
 * 접수는 계속 202를 주고 HTTP 에러율도 안 오르는데 돈만 안 움직이는 상태가 되기 때문이다.
 * k6가 재는 "종결 지연"이 늘어나는 건 보이지만, 어디서 늘어나는지는 이 값이 답한다.
 *
 *   - 0 근처에서 머문다 → 릴레이가 따라가고 있다. 병목은 그 뒤(Kafka·컨슈머·DB)에 있다.
 *   - 계속 올라간다 → 릴레이가 못 따라간다. 폴링 간격(500ms)과 배치 크기(100)가 상한이다.
 */
@Component
public class OutboxBacklogMetrics {
	...
	/**
	 * 게이지 값은 스크랩 시점에 계산된다. 즉 이 메서드는 Prometheus가 긁을 때마다
	 * (5초에 한 번) DB에 {@code COUNT} 한 번을 던진다. 인덱스가 있는 단순 카운트라 감당할 만하고,
	 * 그 대가로 항상 지금 값을 본다.
	 *
	 * 예외를 삼키는 이유가 중요하다. 여기서 예외가 나가면 스크랩 전체가 실패해
	 * JVM·커넥션 풀·컨슈머 지표까지 통째로 사라진다. DB가 죽었을 때가 바로 그 지표들이
