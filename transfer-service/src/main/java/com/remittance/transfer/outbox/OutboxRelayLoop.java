 ...
 * 신호를 기다리는 동안 스레드를 쥐고 있어야 한다. 이 서비스의 스케줄러는 스레드가 하나라, 거기서 기다리면
 * 보관 기간 정리({@code OutboxRetention})가 영영 못 돈다. 2026-08-27에 account에서 스케줄러 스레드를
 ...
 *
 * account-service의 같은 이름 클래스와 구조가 같다 (공유 모듈을 두지 않는다).
 */
@Component
