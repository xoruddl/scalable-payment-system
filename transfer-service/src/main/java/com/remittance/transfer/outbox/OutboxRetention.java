...
/**
 * 발행이 끝난 Outbox 행을 보관 기간이 지나면 지운다.
 *
 * 왜 필요한가 — 2026-08-29에 실제로 당했다
 * 릴레이는 미발행 건만 읽고 나머지는 {@code publishedAt}만 찍는다. 그래서 발행이 끝난 행이
 * 영원히 남는다. 사흘 측정하는 동안 이렇게 됐다.
 *
 ...
 *
 * 같은 100 TPS에서 종결 p99가 4,673ms → 5,044ms로 SLO를 넘었다.
 * 코드는 그대로였고 쌓인 것 말고 바뀐 게 없었다.
 *
 * 보관 기간은 어디서 나오나
 * 발행이 끝난 행이 쓸모 있는 경우는 하나다 — 사고를 조사할 때 "그 이벤트가 실제로
 * 발행됐나"를 확인하는 것. 그러니 사고가 가장 늦게 발견되는 시점보다 길면 된다.
 *
 *   DLT 적재        즉시 (메트릭·WARN)
 *   미종결 송금     2분 (대사)
 *   모르는 돈       5분 (대사)
 *   전체 잔액 대사  EOD 배치로 갈 예정 — 하루
 *
 * 가장 늦은 것이 하루이므로, 사람이 조사할 시간을 더해 3일로 잡았다.
 * 숫자를 늘리고 싶으면 그 이유가 위 표에 있어야 한다.
 *
 * 지우는 것도 부하다
 * 2026-08-29에 360만 건을 한 번에 지우고 곧바로 재봤더니 종결 p99가 11,768ms로
 * 오히려 두 배 나빠졌다. InnoDB가 뒤에서 삭제 흔적을 치우는 중이었기 때문이다
 * (`History list length`). 그래서 작게 끊어서 지우고, 측정 중에는 끌 수 있게 둔다 —
...
public class OutboxRetention {
	...
	/**
	 * 삭제는 별도 빈이 한다. 청크 하나가 트랜잭션 하나여야 하는데, 같은 빈 안에서
	...
	/**
	 * 발행이 끝났는데 아직 남아 있는 건수.
	 *
	 * 이 값이 없어서 못 봤다. 미발행 적체(`remittance.outbox.backlog`)는 처음부터 보고 있었는데,
	 * 발행이 끝난 뒤 쌓이는 것은 아무도 세지 않았다. 그래서 120만 건이 될 때까지 몰랐다.
	 */
	@PostConstruct
