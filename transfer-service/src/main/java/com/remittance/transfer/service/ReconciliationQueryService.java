...
/**
 * 대사에 필요한 조회만 모아둔다. 고치는 일은 하지 않는다.
 *
 * Step 6b에서 송금에 멱등성 키가 남게 되어, 묶인 키가 어느 쪽 사고인지 구분할 수 있게
 * 됐다 — 그 키로 커밋된 송금이 있는지 보면 된다. 그래도 여기서 풀지는 않는다.
 * 키를 푸는 건 그 데이터의 주인인 접수 경로가 할 일이고, 실제로 재요청이 들어오면
 * {@code TransferService}가 같은 근거로 스스로 푼다. 대사는 구분해서 알리는 데까지다.
 *
 * 대사가 직접 풀면 아무도 재요청하지 않은 키까지 건드리게 되고, 그건 사고를 없애는 게 아니라
 * 사고의 흔적을 없애는 것이다.
 */
@Service
