...
/**
 * 송금 상태 전이 한 건을 하나의 트랜잭션으로 실행한다.
 *
 * 재시도(낙관적 락 충돌)를 하려면 트랜잭션 밖에서 다시 시도해야 한다. 같은 트랜잭션
...
public class TransferStateUpdater {
	 ...
	 *
	 * 직전 단계가 아니어도 진행도가 앞서면 건너뛰어서라도 적용한다. 뒤 단계 이벤트가
	...
	/**
	 * 상대 은행 결과를 모르는 상태로 표시한다 (Phase 6.5).
	 *
	 * 종결된 송금은 건드리지 않는다 — 뒤늦게 도착한 이벤트가 닫힌 송금을 다시 열면
