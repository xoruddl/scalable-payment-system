 ...
 *
 * Step 6b 전에는 키와 시각만 실어 보냈다. 그것만으로는 어느 쪽 사고인지 알 수 없어
 * 대사가 "묶인 키가 있다"는 말밖에 못 했다.
 *
 * @param committedTransferId 이 키로 실제로 커밋된 송금이 있으면 그 ID.
 *                              - 있다 — 접수는 끝났고 키에 적기 직전에 죽었다.
 *                                  재요청하면 그 송금을 그대로 돌려받는다(전진 복구).
 *                              - 없다 — 접수가 커밋되지 않았다. 재요청하면 키가 풀리고 새로 접수된다.
