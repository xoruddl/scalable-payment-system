...
/**
 * 이 서비스가 소비하는 Saga 이벤트의 토픽명과 본문.
 * 발행하는 쪽은 {@link com.remittance.transfer.outbox.TransferEventType}에 있다.
 *
 * 서비스 간 공유 모듈을 두지 않기로 했으므로 account/ledger 서비스도 같은 계약을 각자 정의한다.
 * 필드 이름이 곧 계약이니 바꿀 때는 세 곳을 함께 확인해야 한다.
 *
 * 본문에 {@code @JsonIgnoreProperties(ignoreUnknown = true)}를 붙여, 발행하는 쪽이 필드를
...
public final class TransferEvents {
	 ...
	 * 실패가 아니라 제3의 상태다. 실패로 처리하면 이미 나간 돈을 환불해 이중 지급이 된다.
	 */
